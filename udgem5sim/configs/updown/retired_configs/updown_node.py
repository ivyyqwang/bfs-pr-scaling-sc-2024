...
"""
msizes = re.findall(r'\d+', args.mem_size)
stack_addr_size = 2**(30 + math.log2(int(int(msizes[0])/nstacks)))
print(int(stack_addr_size))
node_stack_addr = [(int(i*stack_addr_size), int((i+1)*stack_addr_size)) for i in range(nstacks)]
print(node_stack_addr)
...
for i in range(nnodes):
    if not args.fast_forward:
        systems[i] = System(cpu = [TunedCPU(cpu_id=i) for i in range(np)],
                    mem_mode = 'timing',
                    mem_ranges = [AddrRange(node_stack_addr[i]) for i in range(nstacks)],
    ...
    for j in range(np):
        if args.smt:
            ...
            systems[i].cpu[j].workload = multiprocesses[j]
    ...
    ud_scratchsize = lmbanksize*nlanes # [*2 to allow for in_stream addressing]
    ud_ctrlsize = nlanes*32 # [*2 to allow for in_stream addressing]

    UpMemorySize = nuds*(lmbanksize*nlanes)
    UpCtrlSize = nuds*(nlanes*32)
    ...
    for j in range(nuds_pnode):
        ud_addr_range.append(AddrRange(start = Addr(udspbase + j * ud_scratchsize),
                    end=Addr(udspbase + (j + 1) * ud_scratchsize)))
        ud_ctrladdr_range.append(AddrRange(start = Addr(udctrlbase + j* ud_ctrlsize),
                    end=Addr(udctrlbase + (j+1)* ud_ctrlsize)))
    ...
    systems[i].bridges = [StackXBar() for x in range(nstacks)]
    systems[i].udbus = [UDXBar() for x in range(nstacks)]
    #for k in range(nstacks):
    #    systems[i].membus[k] = SystemXBar()
    #    systems[i].udbus[k] = UDXBar()

    MemClass = Simulation.setMemClass(args)
    print(MemClass)
    systems[i].system_port = systems[i].membus[0].cpu_side_ports
    CacheConfig_node.config_cache(args, systems[i], np, nuds, nstacks)
    MemConfig_node.config_mem(args, systems[i], nc, nstacks)

    #system.updown.mem_side = system.membus.cpu_side_ports
    for j in range(nuds_pnode):
        for k in range(np):
            systems[i].updown[j].cpu_side[k] = systems[i].cpu[k].l1xbar.mem_side_ports
        for k in range(nc):
            systems[i].updown[j].mem_side[k] = systems[i].bridges[int(j/nuds)].cpu_side_ports
        for k in range(nud_channels):
            systems[i].updown[j].udin_side[k] = systems[i].udbus[int(j/nuds)].mem_side_ports
            systems[i].updown[j].udout_side[k] = systems[i].udbus[int(j/nuds)].cpu_side_ports
    
    for j in range(nstacks):
        for k in range(nstacks):
            systems[i].bridges[j].mem_side_ports = systems[i].membus[k].cpu_side_ports
...
if not(args.checkpoint_restore):
    for process in multiprocesses:
        ...
        process.map(Addr(udspbase),
                    Addr(udspbase),
                    UpMemorySize*nuds_pnode,
                    False)
        process.map(Addr(udctrlbase),
                    Addr(udctrlbase),
                    UpCtrlSize*nuds_pnode,
                    False)
for i in range(nnodes):
    exit_event = Simulation.run(args, root, systems[i], FutureClass, checkpoint_dir, cpt_starttick)
