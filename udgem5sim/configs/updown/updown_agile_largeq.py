...
def segment_manager(args, nparts, seg_start, seg_size, block_size):
    segments = []
        ...
        if(len(lineitems) > 0):
            ...
            if(lineitems[0] == "#define" and g5def == 1):
                ...
                if keyval in config.keys():
                    ...
                    elif lineitems[2] in config:
                        config[keyval] = config[lineitems[2]]
    return config
...
parser.add_argument("--print-mem-map", action="store_true", default=False)
...
parser.add_argument("--gseg-block-size", type=str, default="32KB")
parser.add_argument("--lseg-block-size", type=str, default="32KB")
...
parser.add_argument("--mem-lat", type=str, default='40ns', help='Memory Latency in ns')
...
gblock_size = int(2**12) # set the default to 4K
node_segs = segment_manager(args, nstacks, lseg_start, lsize, block_size)
node_global_segs = segment_manager(args, nnodes*nstacks, gseg_start, gsize, gblock_size)
...
mem_clk_domain = SrcClockDomain(clock =  '2400MHz',
                                       voltage_domain = VoltageDomain(voltage = args.sys_voltage))
...
for i in range(nnodes):
    ...
    systems[i].membus = [CoherentStackXBar(intlv_bits=intlv_bits, intlv_low_bit=7, frontend_latency=2, forward_latency=2,response_latency=2, snoop_response_latency=2, width=4096, clk_domain=mem_clk_domain) for x in range(nstacks)]
    ...
    for j in range(nstacks):
        for k in range(np):
            ...
            cpubridges.append(Bridge(req_size=524288, resp_size=524288, ranges=(lranges), clk_domain=switch_clk_domain))
    systems[i].cpubridge=cpubridges
    # Adding bridges between clxbar and membus as well 
    clx2membridges = [] 
    for j in range(len(systems[i].mem_ctrls)):
        clx2membridges.append(Bridge(req_size=524288, resp_size=524288, ranges=systems[i].mem_ctrls[j].range, clk_domain=mem_clk_domain,delay=mem_lat_half))
    ...
    for j in range(nstacks):
        ...
        clx2ibridges.append(Bridge(req_size=524288, resp_size=524288, ranges=clx2irange, clk_domain=switch_clk_domain,delay='10ns'))
        i2clxbridges.append(Bridge(req_size=524288, resp_size=524288, ranges=i2clxrange, clk_domain=switch_clk_domain,delay='10ns'))
    ...
    if nstacks > 1:
        ...
        for j in range(nstacks):
            ...
            # cluster out bridge 
                ...
                i2ibridges.append(Bridge(req_size=524288, resp_size=524288, ranges=lranges, clk_domain=switch_clk_domain,delay='1ns'))
...
if nnodes > 1:
    ...
    for i in range(nnodes):
        ...
        for j in range(nstacks):
            ...
            lranges.append(AddrRange(node_base_ctrl + j*ud_ctrl_size*nuds, node_base_ctrl + (j+1)*ud_ctrl_size*nuds))
            lranges.append(AddrRange(node_base_stack + j*stack_size, node_base_stack + (j+1)*stack_size))
            nodeinbridges.append(Bridge(req_size=524288, resp_size=524288, ranges=lranges, clk_domain=switch_clk_domain,delay='10ns'))
        for j in range(nstacks):
            nodeoutbridges.append(Bridge(req_size=524288, resp_size=524288, ranges=outranges[i], clk_domain=switch_clk_domain,delay='10ns'))
    ...
    for i in range(nnodes):
        for j in range(node_ports):
            ...
            sysinbridges.append(Bridge(req_size=524288, resp_size=524288, ranges=local_inranges, clk_domain=switch_clk_domain,delay='256ns'))
            sysoutbridges.append(Bridge(req_size=524288, resp_size=524288, ranges=local_outranges, clk_domain=switch_clk_domain,delay='256ns'))
