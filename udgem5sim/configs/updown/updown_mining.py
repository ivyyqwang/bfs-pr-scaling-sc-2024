            ...
            if(lineitems[0] == "#define" and g5def == 1):
                ...
                if keyval in config.keys():
                    if keyval == "DEF_MAPPED_SIZE" or keyval == "DEF_GMAPPED_SIZE":
                        ...
                        config[keyval] = int(lineitems[2].replace("'", "").rstrip('uUlL'), 16)
...
parser.add_argument("--simple_dram_lat", type=str, default='25.9ns', help='DRAM Latency for the SimpleMemory in ns')
...
for i in range(nnodes):
    ...
    for j in range(len(systems[i].mem_ctrls)):
        clx2membridges.append(Bridge(req_size=16384, resp_size=16384, ranges=systems[i].mem_ctrls[j].range, clk_domain=mem_clk_domain,delay=mem_lat_half))
    ...
    for j in range(nstacks):
        ...
        clx2ibridges.append(Bridge(req_size=16384, resp_size=16384, ranges=clx2irange, clk_domain=switch_clk_domain,delay='10ns'))
        i2clxbridges.append(Bridge(req_size=16384, resp_size=16384, ranges=i2clxrange, clk_domain=switch_clk_domain,delay='10ns'))
    ...
    if nstacks > 1:
        ...
        for j in range(nstacks):
            ...
            # cluster out bridge 
                ...
                i2ibridges.append(Bridge(req_size=16384, resp_size=16384, ranges=lranges, clk_domain=switch_clk_domain,delay='1ns'))
