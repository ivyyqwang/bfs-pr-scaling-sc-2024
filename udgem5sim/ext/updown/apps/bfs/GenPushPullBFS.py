...
PULL_SWITCH_AT_ITERATION = 16
PUSH_SWITCH_AT_ITERATION = 16
...
class BFS:
    ...
    def __init__(self, efa: EFAProgram, vertex_struct_size, ctr_base_offset, max_thread_per_lane = 250, debug_flag = False):
        ...
        self.kvmsr_offset       = self.front_evw_offset + WORD_SIZE     # 112
        self.send_buffer_offset = self.kvmsr_offset + 8 * WORD_SIZE     # 176
        self.iteration_offset   = self.kvmsr_offset + 18 * WORD_SIZE    # 256
        self.kvmsr_ops_offset   = self.kvmsr_offset + 24 * WORD_SIZE    # 304
    ...
    def setup_cache(self, cache_base_offset, num_entries, entry_size):
        ...
        self.cache_base_offset  = self.kvmsr_offset + 32 * WORD_SIZE
        self.cache_size         = ((1 << 16) - self.cache_base_offset - 32) // entry_size
    ...
    def gen_pull_kvmsr(self):
        
        
        self.udkvmsr = UDKeyValueMapShuffleReduceTemplate(efa=self.efa, task_name=f"{self.task}_pull", meta_data_offset=self.kvmsr_offset, debug_flag=self.debug_flag)
        
        self.udkvmsr.set_input_kvset(OneDimKeyValueSet(f"{self.task}_input_kvset", element_size=VERTEX_STRUCT_SIZE, argument_size=1))

        self.udkvmsr.set_max_thread_per_lane(max_map_th_per_lane=28)
    ...
    def gen_lm_init(self):
        ...
        '''
        Operands:
            OB_0:   Node 0's vertex array pointer.
            OB_1:   Number of vertices.
            OB_2:   Number of UpDowns.
            OB_3:   Pointer to the partition array (64-bit DRAM address)
            OB_4:   Number of lanes - 1
            OB_5:   Froniter base pointer
            OB_6:   Froniter stride
            OB_7:   Number of partitions per lane
        '''
        if self.debug_flag:
            init_node_tran.writeAction(f"print ' '")
            init_node_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <init_node> ev_word=%lu' {'X0'} {'EQT'}")
            init_node_tran.writeAction(f"print '[DEBUG][NWID %ld] Operands: graph_ptr=0x%lx, num_vertices=%ld, num_uds=%ld, partition_array=0x%lx (PART_PARAM=%ld), ud_mask=%ld, front=0x%lx, front_stride=0x%lx' \
                {'X0'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+1}'} {f'X{OB_REG_BASE+2}'} {f'X{OB_REG_BASE+3}'} {f'X{OB_REG_BASE+7}'} {f'X{OB_REG_BASE+4}'} {f'X{OB_REG_BASE+5}'} {f'X{OB_REG_BASE+6}'}")
        ...
        init_node_tran.writeAction(f"lshift {f'X{OB_REG_BASE+6}'} {front_stride} {1}")
        ...
        init_node_tran.writeAction(f"sendr3_wcont {init_ud_evw} {fin_init_evw} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+4}'} {f'X{OB_REG_BASE+2}'}")
        ...
        init_node_tran.writeAction(f"movir {lm_base} {self.kvmsr_ops_offset}")
        init_node_tran.writeAction(f"add {'X7'} {lm_base} {lm_base}")
        init_node_tran.writeAction(f"movrl {f'X{OB_REG_BASE+3}'} 0({lm_base}) 0 8")
        if self.debug_flag:
            init_node_tran.writeAction(f"print '[DEBUG][NWID %d] Partition array %lu(0x%lx) and number of partition per lane = %ld' {'X0'} {f'X{OB_REG_BASE+3}'} {f'X{OB_REG_BASE+3}'} {f'X{OB_REG_BASE+7}'}")
        init_node_tran.writeAction(f"movrl {f'X{OB_REG_BASE+7}'} 8({lm_base}) 0 8")
        init_node_tran.writeAction(f"muli {f'X{OB_REG_BASE+2}'} {nwid} {LANE_PER_UD}")
        init_node_tran.writeAction(f"movrl {nwid} 16({lm_base}) 0 8")
        init_node_tran.writeAction(f"addi {'X7'} {self.scratch[0]} {self.udkvmsr.in_kvset_offset}")
        init_node_tran.writeAction(f"movrl {self.scratch[0]} 24({lm_base}) 0 8")
        if self.debug_flag:
            init_node_tran.writeAction(f"print '[DEBUG][NWID %d] Save input key value set base pointer %lu(0x%lx) and size %ld to scratchpad addr %ld(0x%lx)' " + 
                                       f"{'X0'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+1}'} {self.scratch[0]} {self.scratch[0]}")
        init_node_tran.writeAction(f"movrl {f'X{OB_REG_BASE+0}'} 0({self.scratch[0]}) 1 8")
        init_node_tran.writeAction(f"movrl {f'X{OB_REG_BASE+1}'} 0({self.scratch[0]}) 1 8")
        ...
        if self.debug_flag:
            ...
            init_ud_tran.writeAction(f"print '[DEBUG][NWID %ld] Operands: ud_bin_ptr=0x%lx, udid=%ld, log2_num_uds=%ld' {'X0'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+1}'} {f'X{OB_REG_BASE+2}'}")
    ...
    def gen_global_broadcast(self):
        
        self.init_iter_ev_label     = self.__get_event_mapping("initialize_iteration")
        self.bcst_fetch_ev_label    = self.__get_event_mapping("broadcast_frontier_fetch")
        self.ud_mstr_sync_ev_label      = self.__get_event_mapping("finish_fetch_return")
        self.switch_frontiers_ev_label  = self.__get_event_mapping("switch_frontiers")
        self.switch_sync_ev_label   = self.__get_event_mapping("terminate_sync")
        ...
        init_frontier_label = "init_frontier"
        pull_phase_label = "pull_phase"
        push_phase_label = "push_phase"
        init_iter_tran  = self.state.writeTransition("eventCarry", self.state, self.state, self.init_iter_ev_label)
        if self.debug_flag:
            init_iter_tran.writeAction(f"print ' '")
            init_iter_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <initialize_iteration> Start %ld iteration BFS. Number of nodes = %ld, number of updowns per node = %ld.' {'X0'} {'X10'} {'X8'} {'X9'}")
        init_iter_tran.writeAction(f"addi {'X0'} {nwid} 0")
        init_iter_tran.writeAction(f"addi {f'X{OB_REG_BASE+0}'} {num_nodes} 0")
        init_iter_tran.writeAction(f"addi {f'X{OB_REG_BASE+1}'} {num_ud_per_node} 0")
        init_iter_tran.writeAction(f"addi {f'X{OB_REG_BASE+2}'} {num_iter} 1")
        init_iter_tran.writeAction(f"perflog 1 {0} 'Start %ld iteration BFS. ' {num_iter} ")
        init_iter_tran.writeAction(f"mul {num_nodes} {num_ud_per_node} {num_uds}")
        set_ev_label(init_iter_tran, sync_evw, self.bcst_fetch_ev_label)
        init_iter_tran.writeAction(f"beqi {num_iter} {1} {init_frontier_label}")
        # init_iter_tran.writeAction(f"bgei {num_iter} {PULL_SWITCH_AT_ITERATION} {pull_phase_label}")
        init_iter_tran.writeAction(f"{push_phase_label}: movir {ud_ctr} 1")
        init_iter_tran.writeAction(f"sendr_wcont {sync_evw} {sync_evw} {num_iter} {ud_ctr}")
        init_iter_tran.writeAction(f"yield")
        
        # Switch to pull phase
        init_iter_tran.writeAction(f"{pull_phase_label}: bgti {num_iter} {PUSH_SWITCH_AT_ITERATION} {push_phase_label}")
        if self.debug_flag:
            init_iter_tran.writeAction(f"print '[DEBUG][NWID %ld] Switch to pull phase at iteration %ld' {'X0'} {num_iter}")
        init_iter_tran.writeAction(f"addi {'X7'} {lm_base} {self.iteration_offset}")
        init_iter_tran.writeAction(f"move {num_iter} {0}({lm_base}) 0 8")
        init_iter_tran.writeAction(f"addi {'X7'} {lm_base} {self.kvmsr_ops_offset}")
        init_iter_tran.writeAction(f"evii {self.ev_word} {self.udkvmsr.init_kvmsr_ev_label} 255 5")
        init_iter_tran.writeAction(f"send_wret {self.ev_word} {self.switch_frontiers_ev_label} {lm_base} 5 {self.scratch[0]}")
        init_iter_tran.writeAction(f"mov_imm2reg {nfront_size} {0}")
        init_iter_tran.writeAction(f"yield")
        
        # Initialize the frontier
        init_iter_tran.writeAction(f"{init_frontier_label}: movir {ud_ctr} 0")
        set_ev_label(init_iter_tran, self.ev_word, self.init_front_ev_label, new_thread=True)
        set_nwid(init_iter_tran, self.ev_word, nwid, src_ev=self.ev_word, label=bcst_loop_label)
        init_iter_tran.writeAction(f"sendr_wcont {self.ev_word} {sync_evw} {num_iter} {ud_ctr} ")
        init_iter_tran.writeAction(f"addi {ud_ctr} {ud_ctr} 1")
        init_iter_tran.writeAction(f"addi {nwid} {nwid} {LANE_PER_UD}")
        init_iter_tran.writeAction(f"blt {ud_ctr} {num_uds} {bcst_loop_label}")
        init_iter_tran.writeAction(f"mov_imm2reg {num_updates} {0}")
        init_iter_tran.writeAction(f"mov_imm2reg {nfront_size} {0}")
        init_iter_tran.writeAction(f"yield")
        
        '''
        Finish the initialization of the frontier and start the BFS iteration
        '''
        ...
        set_ev_label(bcst_fetch_tran, sync_evw, self.ud_mstr_sync_ev_label)
        ...
        bcst_fetch_tran.writeAction(f"movir {num_updates} {0}")
        ...
        ud_mstr_sync_tran = self.state.writeTransition("eventCarry", self.state, self.state, self.ud_mstr_sync_ev_label)
        if self.debug_flag:
            ud_mstr_sync_tran.writeAction(f"print ' '")
            ud_mstr_sync_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <ud_mstr_finish_fetch_return> ev_word=%lu Updown %ld sends out num_updates=%ld pending_ack=%ld' {'X0'} {'EQT'} {f'X{OB_REG_BASE+1}'} {f'X{OB_REG_BASE+0}'} {ud_ctr}")
        ud_mstr_sync_tran.writeAction(f"add {f'X{OB_REG_BASE+0}'} {num_updates} {num_updates}")
        ud_mstr_sync_tran.writeAction(f"subi {ud_ctr} {ud_ctr} 1")
        ud_mstr_sync_tran.writeAction(f"beqi {ud_ctr} {0} {fin_fetch_label}")
        ud_mstr_sync_tran.writeAction(f"yield")
        # Finish sending out update requests in all updown's frontier, start global synchronization
        ud_mstr_sync_tran.writeAction(f"{fin_fetch_label}: beqi {num_updates} {0} {zero_front_label}")
        if ITERATION_INFO_FLAG:
            ud_mstr_sync_tran.writeAction(f"print '[DEBUG][NWID %ld] Finish fetching all the frontiers (= %ld) on iteration %ld. Frontier size = %ld. Start synchronization.' \
                                 {'X0'} {num_uds} {num_iter} {num_updates}")
            ud_mstr_sync_tran.writeAction(f"perflog 1 {2} 'Finish fetching all the frontiers on iteration %ld. Frontier size = %ld. Start synchronization.' {num_iter} {num_updates}")
        set_ev_label(ud_mstr_sync_tran, self.ev_word, self.global_sync_init_ev_label, new_thread=True)
        set_ev_label(ud_mstr_sync_tran, self.scratch[0], self.switch_frontiers_ev_label)
        ud_mstr_sync_tran.writeAction(f"sendr3_wcont {self.ev_word} {self.scratch[0]} {num_nodes} {num_ud_per_node} {num_iter}")
        ud_mstr_sync_tran.writeAction(f"yield")
        # All frontiers are empty, return to top program.
        ud_mstr_sync_tran.writeAction(f"{zero_front_label}: movir {self.scratch[0]} {1}")
        ud_mstr_sync_tran.writeAction(f"addi {'X7'} {lm_base} 0")
        ud_mstr_sync_tran.writeAction(f"move {self.scratch[0]} {self.top_flag_offset}({lm_base}) 0 8")
        ud_mstr_sync_tran.writeAction(f"move {num_iter} {self.top_val_offset}({lm_base}) 0 8")
        if ITERATION_INFO_FLAG:
            ud_mstr_sync_tran.writeAction(f"print '[DEBUG][NWID %ld] BFS converges at iteration = %ld. Return to top.' {'X0'} {num_iter}")
        ud_mstr_sync_tran.writeAction(f"yieldt")
        
        '''
        Frontier master return, finish iteration
        X8:     iteration number
        '''
        switch_frontier_tran = self.state.writeTransition("eventCarry", self.state, self.state, self.switch_frontiers_ev_label)
        if self.debug_flag:
            switch_frontier_tran.writeAction(f"print '[DEBUG][NWID %ld] Finish global sync on iter %ld, switching all frontiers' {'X0'} {num_iter}")
        set_ev_label(switch_frontier_tran, self.ev_word, self.term_ud_mstr_ev_label, new_thread=True)
        set_ev_label(switch_frontier_tran, sync_evw, self.switch_sync_ev_label)
        switch_frontier_tran.writeAction(f"addi {'X0'} {nwid} 0")
        # set_ev_label(term_sync_tran, sync_evw, self.switch_sync_ev_label)
        set_nwid(switch_frontier_tran, self.ev_word, nwid, src_ev=self.ev_word, label=bcst_loop_label)
        switch_frontier_tran.writeAction(f"sendr_wcont {self.ev_word} {sync_evw} {num_iter} EQT")
        switch_frontier_tran.writeAction(f"addi {ud_ctr} {ud_ctr} 1")
        switch_frontier_tran.writeAction(f"addi {nwid} {nwid} {LANE_PER_UD}")
        if self.debug_flag:
            switch_frontier_tran.writeAction(f"print '[DEBUG][NWID %ld] Finish fetching all frontiers terminate the frontier master on udid %ld' {'X0'} {ud_ctr}")
        switch_frontier_tran.writeAction(f"blt {ud_ctr} {num_uds} {bcst_loop_label}")
        switch_frontier_tran.writeAction(f"movir {nfront_size} {0}")
        switch_frontier_tran.writeAction(f"yield")
        ...
        '''
        term_sync_tran = self.state.writeTransition("eventCarry", self.state, self.state, self.switch_sync_ev_label)
        if self.debug_flag:
        ...
        if ITERATION_INFO_FLAG:
            term_sync_tran.writeAction(f"print '[DEBUG][NWID %ld] Finish switching all the frontiers on iteration %ld. New frontier size = %ld' {'X0'} {num_iter} {nfront_size}")
            term_sync_tran.writeAction(f"perflog 1 {2} 'Finish switching all the frontiers on iteration %ld. New frontier size = %ld. ' {num_iter} {nfront_size}")
        term_sync_tran.writeAction(f"beqi {nfront_size} {0} {zero_front_label}")
        set_ev_label(term_sync_tran, self.scratch[0], self.init_iter_ev_label)
        ...
        if ITERATION_INFO_FLAG:
            ...
            term_sync_tran.writeAction(f"perflog 1 {2} ' BFS converges at iteration = %ld. Return to top.' {num_iter}")
    ...
    def gen_frontier_master(self):
        ...
        self.insert_front_ev_label = self.__get_event_mapping("insert_frontier")
        self.insert_ack_ev_label   = self.__get_event_mapping("insert_ack")
        self.alloc_front_ev_label  = self.__get_event_mapping("allocate_frontier")
        ...
        self.switch_ud_front_ev_label   = self.__get_event_mapping("switch_ud_frontier")
        ...
        set_ev_label(init_front_tran, alloc_evw, self.alloc_front_ev_label)
        ...
        init_front_tran.writeAction(f"movrl {alloc_evw} {self.front_evw_offset}({lm_addr}) 0 8")
        ...
        OB_0:   number of elements to insert
        '''
        alloc_tran = self.state.writeTransition("eventCarry", self.state, self.state, self.alloc_front_ev_label)
        if self.debug_flag:
            alloc_tran.writeAction(f"print ''")
            alloc_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <allocate_frontier> ev_word=%lu continuation = %lu Operands: num_elements=%ld request_vid=%ld' {'X0'} {'EQT'} {'X1'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+1}'} ")
        alloc_tran.writeAction(f"sli {f'X{OB_REG_BASE+0}'} {val} {LOG2_WORD_SIZE}")
        alloc_tran.writeAction(f"sendr_wcont {'X1'} {alloc_evw} {nfront_ptr} {val} EQT")
        if self.debug_flag:
            alloc_tran.writeAction(f"sub {nfront_ptr} {nfront_base} {num_element}")
            alloc_tran.writeAction(f"sri {num_element} {num_element} {LOG2_WORD_SIZE}")
            alloc_tran.writeAction(f"print '[DEBUG][NWID %ld] Allocate %ld elements in frontier starting at at addr %lu(0x%lx), frontier_base_addr = %lu(0x%lx) " + 
                                    f"frontier_size = %ld'  {'X0'} {f'X{OB_REG_BASE+0}'} {nfront_ptr} {nfront_ptr} {nfront_base} {nfront_base} {val}")
        alloc_tran.writeAction(f"add {nfront_ptr} {val} {nfront_ptr}")
        if self.debug_flag:
            alloc_tran.writeAction(f"print '[DEBUG][NWID %ld] New frontier address %lu(0x%lx)' {'X0'} {nfront_ptr} {nfront_ptr}")
        alloc_tran.writeAction(f"yield")
        ...
        continue_label = "continue"
        switch_front_tran = self.state.writeTransition("eventCarry", self.state, self.state, self.switch_ud_front_ev_label)
        
        # if self.debug_flag:
        #     fin_rd_front_tran.writeAction(f"print '[DEBUG][NWID %ld][{self.fin_fetch_front_ev_label}] Event <switch_frontier> ev_word=%lu " + 
        #                                   f"lane %ld frontier master switch frontier at iteration %ld' {'X0'} {'EQT'} {'X0'} {'X8'} {'X9'}")
        # Finish insert all the entries into the frontier, given master the frontier for next iteration.
        switch_front_tran.writeAction(f"addi {'X7'} {lm_addr} 0")
        switch_front_tran.writeAction(f"sub {nfront_ptr} {nfront_base} {val}")
        switch_front_tran.writeAction(f"sri {val} {val} {LOG2_WORD_SIZE}")
        if self.debug_flag:
            switch_front_tran.writeAction(f"print '[DEBUG][NWID %ld][{self.switch_ud_front_ev_label}] Event <switch_frontier> ev_word=%lu finish reading frontier at " + 
                                          f"iteration %ld store new frontier pointer %lu(0x%lx) base %lu(0x%lx) frontier_size=%ld' {'X0'} {'EQT'} {'X8'} {nfront_ptr} {nfront_ptr} {nfront_base} {nfront_base} {val}")
        switch_front_tran.writeAction(f"sendr3_wcont {'X1'} {alloc_evw} {'X8'} {val} {nfront_base}")
        # Exchange the frontier and clear the new frontier to be inserted.
        switch_front_tran.writeAction(f"addi {ofront_base} {val} 0")
        switch_front_tran.writeAction(f"addi {nfront_base} {ofront_base} 0")
        switch_front_tran.writeAction(f"addi {val} {nfront_base} 0")
        switch_front_tran.writeAction(f"addi {nfront_ptr} {ofront_ptr} 0")
        switch_front_tran.writeAction(f"addi {nfront_base} {nfront_ptr} 0")
        switch_front_tran.writeAction(f"move {ofront_base} {self.ofront_base_offset}({lm_addr}) 0 8")
        switch_front_tran.writeAction(f"move {ofront_ptr} {self.ofront_ptr_offset}({lm_addr}) 0 8")
        switch_front_tran.writeAction(f"move {nfront_base} {self.nfront_base_offset}({lm_addr}) 0 8")
        switch_front_tran.writeAction(f"move {nfront_base} {self.nfront_ptr_offset}({lm_addr}) 0 8")
        if self.debug_flag:
            switch_front_tran.writeAction(f"print '[DEBUG][NWID %ld][{self.switch_ud_front_ev_label}] After switch frontier, ofront_base=%lu(0x%lx), ofront_ptr=%lu(0x%lx), nfront_base=%lu(0x%lx), " + 
                                          f"nfront_ptr=%lu(0x%lx)' {'X0'} {ofront_base} {ofront_base} {ofront_ptr} {ofront_ptr} {nfront_base} {nfront_base} {nfront_ptr} {nfront_ptr}")
        switch_front_tran.writeAction(f"yield")
    ...
    def gen_ud_frontier(self):
        ...
        set_ev_label(term_ud_tran, self.ev_word, self.switch_ud_front_ev_label, src_ev=self.ev_word)
        term_ud_tran.writeAction(f"sendr_wcont {self.ev_word} {'X1'} {'X8'} {'X9'}")
        term_ud_tran.writeAction("yield_terminate")

    def gen_lane_master(self):
    ...
    def gen_update_neighbor(self):
        ...
        NEIGHBOR_FETCH_MAX_REQUESTS = 16
        ...
        if self.debug_flag:
            ...
            fetch_vert_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <fetch_vertex> dram_addr=%lu(0x%lx) vid=%ld ori_vid=%ld " + 
                                        f"distance=%ld degree=%ld nlist==%lu(0x%lx)' {'X0'} {f'X{3}'} {f'X{3}'} {vid_op} {orig_vid_op} {dist_op} {deg_op} {nlist_op} {nlist_op}")
        ...
        fetch_vert_tran.writeAction(f"{root_v_label}: addi {deg_op} {degree} 0")
        ...
        # fetch_vert_tran.writeAction(f"subi {degree} {self.scratch[1]} 1")      # no need to update the parent
        fetch_vert_tran.writeAction(f"addi {'X7'} {self.scratch[0]} 0")
        ...
        # fetch_vert_tran.writeAction(f"{zero_deg_label}: addi {'X7'} {self.scratch[0]}  0")
        # fetch_vert_tran = self.__incre_counter(fetch_vert_tran, self.num_threads_offset, '-1', self.scratch[2], self.scratch[0])
        ...
        for k in range(DRAM_MSG_SIZE):
            ...
            # fetch_nbor_tran.writeAction(f"print '[DEBUG][NWID %ld] Update vid=%ld' {'X0'} X{OB_REG_BASE+k}")
        ...
        # fetch_nbor_tran.writeAction(f" addi {'X7'} {self.scratch[0]} 0")
        # fetch_nbor_tran = self.__incre_counter(fetch_nbor_tran, self.num_threads_offset, '-1', self.scratch[2], self.scratch[0])
    ...
    def gen_update_dist(self):
        ...
        # udp_dist_tran.writeAction(f"andi {cached_key} {self.scratch[0]} {self.INACTIVE_MASK - 1}")
        ...
        set_ev_label(read_v_tran, store_ack_evw, self.store_ack_ev_label)
        ...
        read_v_tran.writeAction(f"sub {sibling_max_vid} {sibling_vid} {num_siblings}")
        read_v_tran.writeAction(f"add {pending_ack} {num_siblings} {pending_ack}")
        read_v_tran.writeAction(f"add {pending_ack} {num_siblings} {pending_ack}")
        if self.debug_flag:
            read_v_tran.writeAction(f"print '[DEBUG][NWID %ld] Update siblings of vid=%ld, sibling_vid_range=[%ld, %ld], " + 
                                    f"num_siblings=%d' {'X0'} {f'X{OB_REG_BASE+2}'} {sibling_vid} {sibling_max_vid} {num_siblings}")
        read_v_tran.writeAction(f"movlr {self.front_evw_offset}({'X7'}) {front_evw} 0 8")
        if self.debug_flag:
            read_v_tran.writeAction(f"print '[DEBUG][NWID %ld] Request %ld elements from frontier evw=%lu' {'X0'} {num_siblings} {front_evw} ")
        read_v_tran.writeAction(f"sendr3_wret {front_evw} {self.insert_front_ev_label} {num_siblings} {self.vid} {self.parent} {self.ev_word}")
        self.__get_v_dist_addr(read_v_tran, sibling_vid, vertex_addr)
        read_v_tran.writeAction(f"{ins_sibling_label}: sendr2_dmlm {vertex_addr} {store_ack_evw} {self.dist} {self.parent}")
        if self.debug_flag:
            read_v_tran.writeAction(f"print '[DEBUG][NWID %ld][{self.read_v_ev_label}] Update sibling vid = %ld distance and parent vid' {'X0'} {sibling_vid}")
        read_v_tran.writeAction(f"addi {sibling_vid} {sibling_vid} {1}")
        read_v_tran.writeAction(f"addi {vertex_addr} {vertex_addr} {VERTEX_STRUCT_BSIZE}")
        ...
        read_v_tran.writeAction(f"{skip_sibling_label}: movlr {self.front_evw_offset}({'X7'}) {front_evw} 0 8")
        read_v_tran.writeAction(f"movir {num_siblings} {1}")
        read_v_tran.writeAction(f"addi {self.vid} {sibling_vid} 0")
        read_v_tran.writeAction(f"sendr_wret {front_evw} {self.insert_front_ev_label} {num_siblings} {self.vid} {self.ev_word}")
        if self.debug_flag:
        ...
        '''
        Event <store_ack> to acknowledge the update.
        '''
        ...
        if self.debug_flag:
            store_ack_tran.writeAction(f"print '[DEBUG][NWID %ld] Finish update vid=%ld new_distance=%ld' {'X0'} {self.vid} {self.dist}")
        ...
        Insert vertex into frontier.
        OB_0:   frontier ptr
        OB_1:   number of bytes allocated
        '''
        sibling_vid_bound = self.scratch[0]
        insert_frontier_label = "insert_to_frontier"
        ...
        if self.debug_flag:
            ...
            insert_tran.writeAction(f"print '[DEBUG][NWID %ld] Event <insert_to_frontier> ev_word=%lu num_siblings = %lu Operands: vid=%ld' {'X0'} {'EQT'} {num_siblings} {f'X{OB_REG_BASE+0}'}")
        insert_tran.writeAction(f"addi {f'X{OB_REG_BASE+0}'} {front_ptr} 0")
        insert_tran.writeAction(f"add {sibling_vid} {num_siblings} {sibling_vid_bound}")
        if self.debug_flag:
            insert_tran.writeAction(f"print '[DEBUG][NWID %ld] Insert split vids from vid=%ld to vid=%ld " +
                                    f"number of element inserted = %ld pending_ack = %ld' {'X0'} {sibling_vid} {sibling_vid_bound} {num_siblings} {num_siblings}")
        insert_tran.writeAction(f"{insert_frontier_label}: sendr_dmlm {front_ptr} {store_ack_evw} {sibling_vid}")
        insert_tran.writeAction(f"addi {sibling_vid} {sibling_vid} 1")
        insert_tran.writeAction(f"addi {front_ptr} {front_ptr} {WORD_SIZE}")
        insert_tran.writeAction(f"blt {sibling_vid} {sibling_vid_bound} {insert_frontier_label}")
        if self.debug_flag:
            insert_tran.writeAction(f"print '[DEBUG][NWID %ld] Insert vertex %ld to frontier at addr range [%lu(0x%lx), %lu(0x%lx)] " + 
                                    f"number of element inserted = %ld pending_ack = %ld' {'X0'} {f'X{OB_REG_BASE+0}'} {f'X{OB_REG_BASE+0}'} {front_ptr} {front_ptr} {pending_ack}")
    ...
    def __get_v_dist_addr(self, tran: EFAProgram.Transition, vid: str, addr: str, label: str = '') -> EFAProgram.Transition:
        
        label = f'{label}: ' if label else ''
        tran.writeAction(f"{label}move {self.vertices_ptr_offset}(X7) {self.graph_ptr} 0 8")
    ...
    def gen_global_sync(self):
        ...
        self.global_sync_init_ev_label   = self.__get_event_mapping("init_global_sync")
        ...
        global_sync = GlobalSync("push_pull_bfs", self.state, self.ev_word, global_sync_offsets, self.scratch, self.debug_flag_term)

        global_sync.set_labels(self.global_sync_init_ev_label, self.global_sync_ev_label, self.node_init_ev_label,
...
def GenerateSyncBfsEFA(efa: EFAProgram):
    ...
    bfs_generator.setup_cache(cache_base_offset=384, num_entries=2035, entry_size=32)

    bfs_generator.gen_pull_kvmsr()
    bfs_generator.gen_lm_init()
    bfs_generator.gen_global_sync()
    bfs_generator.gen_frontier_master()
    bfs_generator.gen_ud_frontier()
    bfs_generator.gen_lane_master()
    bfs_generator.gen_global_broadcast()
