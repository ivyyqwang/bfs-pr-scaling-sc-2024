...
#define USAGE "USAGE: ./updown_bfs <graph_file> <num_uds> <root_vid> (<output_file>) "
...
int main(int argc, char *argv[]) {
...
#else
#ifdef BASIM
  UpDown::BASimUDRuntime_t *bfs_rt = new UpDown::BASimUDRuntime_t(machine, "bfs_exe.bin", 0, 1);
#else
...
#ifdef DEBUG
    printf("Vertex vid=%ld - deg %ld, dist %ld, "
           "ori_vid %ld, split_range [%ld, %ld], neigh_list %p\nNeighbors: [",
           temp_vertex->vid, temp_vertex->degree, g_v_bin[i].distance, temp_vertex->orig_vid, temp_vertex->split_range >> 32,
           temp_vertex->split_range & 0xffffffff, temp_vertex->neighbors);
  ...
  // Init scratchpad
  init_scratchpad(bfs_rt, g_v_bin, front_bins[0], num_split_vertices, num_uds, front_stride);
