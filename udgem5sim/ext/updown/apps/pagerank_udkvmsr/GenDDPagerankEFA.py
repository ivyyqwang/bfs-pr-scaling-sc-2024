...
def GenMSRPagerankEFA(efa: EFAProgram):
    ...
    pagerankMSR.setup_cache(cache_offset=72+32*8, num_entries=4000, entry_size=2)
    ...
    pagerankMSR.set_max_thread_per_lane(max_map_th_per_lane=32, max_reduce_th_per_lane=218)
    ...
    init_tran.writeAction(f"send_wret {ev_word_reg} {'updown_terminate'} {send_buffer} 5 {temp_reg}")
...
def kv_map(pagerankMSR: PagerankMapShuffleReduce) -> int:
    ...
    alpha_offset = pagerankMSR.in_kvset_offset + WORD_SIZE * 2
    ...
    alpha     = "UDPR_5"    # Register UDPR_5 stores the alpha value
    ...
    tran.writeAction(f"movlr {alpha_offset}({lm_base}) {alpha} 0 8")
    tran.writeAction(f"movir {scratch[0]} 1")
    tran.writeAction(f"fcnvt.i64.64 {scratch[0]} {scratch[0]}")
    tran.writeAction(f"fadd.64 {pr_value} {scratch[0]} {pr_value}")
    tran.writeAction(f"fmul.64 {pr_value} {alpha} {new_pr_value}")
    ...
    tran.writeAction(f"fmul.64 {pr_value} {scratch[0]} {new_pr_value}")
    tran.writeAction(f"fdiv.64 {new_pr_value} {degree} {new_pr_value}")
