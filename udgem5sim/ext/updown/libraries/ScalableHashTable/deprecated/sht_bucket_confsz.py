...
class SHTArrayBucket:
    ...
    def _initialize(cls, state: State):
        ...
        # ===== TRAN - init entry =====
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: start' {'X0'}")
        # calc desc lm addr
        ...
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: done' {'X0'}")
    ...
    def initialize(cls, tran: Transition, ret: int, tmp_reg0: str, tmp_reg1: str, arg_lm_addr_reg: str, dst_nwid_reg: str = 'X0') -> None:
        sht_macros.call_wlm(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_init, ret_tran_label=ret, tmp_reg0=tmp_reg0, tmp_reg1=tmp_reg1, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=5)
    ...
    def _update(cls, state: State):
        ...
        # tran.writeAction(f"sendr_dmlm_wret {REG_KEY_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_KEY} {REG_KEY}")  # FIXME
        tran.writeAction(f"sendr_dmlm_wret {REG_KEY_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_KEY} {REG_TMP3}")  # only writing one word
        ...
        for i in range(1, 6):
            tran.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-LB-send_val_{i}'}: send_dmlm_wret {REG_VAL_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_BUF_LM_ADDR} {i} {REG_TMP3}")
            if cls.DEBUG:
        ...
        tran.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-LB-send_ld'}: send_dmlm_ld_wret {REG_LAST_LOAD_ADDR} {tran_key_ld_ret.getLabel()} {8} {REG_TMP3}")
        ...
        # store key
        tran_key_ld_ret.writeAction(f"sendr2_dmlm_wret {REG_KEY_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_KEY} {REG_KEY} {REG_TMP3}")
        if cls.DEBUG:
        ...
        for i in range(1, 6):
            tran_key_ld_ret.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-nomatch_send_val_{i}'}: send_dmlm_wret {REG_VAL_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_BUF_LM_ADDR} {i} {REG_TMP3}")
            if cls.DEBUG:
        ...
        if cls.DEBUG:
            tran_key_ld_ret.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: shouldn't reach here!' {'X0'}")
        for i in range(1, 6):
            tran_key_ld_ret.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-match_send_val_{i}'}: send_dmlm_wret {REG_VAL_WR_ADDR} {tran_key_st_ret.getLabel()} {REG_BUF_LM_ADDR} {i} {REG_TMP3}")
            if cls.DEBUG:
                tran_key_ld_ret.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: writing {i} words value' {'X0'}")
    ...
    def update(cls, tran: Transition, ret: int, tmp_reg0: str, tmp_reg1: str, arg_lm_addr_reg: str, arg_lm_words: int, dst_nwid_reg: str = 'X0') -> None:
        sht_macros.call_wlm(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_update, ret_tran_label=ret, tmp_re0=tmp_reg0, tmp_reg1=tmp_reg1, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=arg_lm_words)
    ...
    def _get(cls, state: State):
        ...
        for i in range(1, 9):
            tran_key_ld_ret.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-match_send_val_{i}'}: send_dmlm_ld_wret {REG_VAL_RD_ADDR} {tran_val_ld_ret.getLabel()} {i} {REG_TMP3}")
            if i <= 5:
    ...
    def _get_fast(cls, state: State):
        ...
        tran_key_ld_ret.writeAction(f"jmp {f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-match_send_val_8'}")  # for value words > 8, load the first 8 words
        for i in range(1, 9):
            tran_key_ld_ret.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-match_send_val_{i}'}: send_dmlm_ld {REG_VAL_RD_ADDR} {REG_CONT} {i} {REG_TMP3}")  # directly return to caller
            if cls.DEBUG:
                tran_key_ld_ret.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: loading {i} words value' {'X0'}")
            tran_key_ld_ret.writeAction(f"jmp {f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-match_send_val_done'}")
