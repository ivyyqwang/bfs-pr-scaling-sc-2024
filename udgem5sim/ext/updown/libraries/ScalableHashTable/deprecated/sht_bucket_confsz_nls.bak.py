...
class SHTArrayBucket:
    ...
    def _update(cls, state: State):
        ...
        tran_key_ld_ret.writeAction(f"add {REG_VAL_WR_ADDR} {REG_TMP1} {REG_VAL_WR_ADDR}")
        # count up the entries
        tran_key_ld_ret.writeAction(f"move {cls.DESC_STURCT_OFF_ENTRY_COUNT}({REG_DESC_LM_ADDR}) {REG_TMP1} 0 8")
    ...
    def _append(cls, state: State):
        ...
        tran.writeAction(f"mov_imm2reg {REG_TMP1} {0}")
        tran.writeAction(f"move {REG_TMP1} {0}({REG_BUF_LM_ADDR}) 0 8")  # set byte counter for the value to 0
        ...
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: shouldn't reach here!' {'X0'}")
        for i in range(1, 6):
            tran.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-LB-send_val_{i}'}: send_dmlm_wret {REG_VAL_WR_ADDR} {tran_key_val_st_ret.getLabel()} {REG_BUF_LM_ADDR} {i + 1}")  # include the byte counter
            if cls.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: writing {i} words value' {'X0'}")
            tran.writeAction("yield")
        ...
        if cls.DEBUG:
            tran_key_ld_ret.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: shouldn't reach here!' {'X0'}")
        for i in range(1, 6):
            tran_key_ld_ret.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-nomatch_send_val_{i}'}: send_dmlm_wret {REG_VAL_WR_ADDR} {tran_key_val_st_ret.getLabel()} {REG_BUF_LM_ADDR} {i + 1}")  # include the byte counter
            if cls.DEBUG:
                tran_key_ld_ret.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] {FN_NAME.upper()}: writing {i} words value' {'X0'}")
            tran_key_ld_ret.writeAction(f"jmp {f'{CLS_NAME}-{FN_NAME}-TR-key_ld_ret-LB-nomatch_send_val_done'}")
        # count up the entries
        ...
        # FIXME: need to load the begining of value first
        """----- Send store value -----"""
        # FIXME: update the byte counter in the first word of value
