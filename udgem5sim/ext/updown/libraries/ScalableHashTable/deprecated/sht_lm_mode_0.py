...
class SHT:
    ...
    def _initialize(cls, state: State):
        ...
        # tmp2 -> current bucket desc lm addr (assume it does not have to start from the first lane of the UD)
        tran.writeAction(f"mov_reg2reg {OB_BUCKET_DESC_LM_OFFSET} {REG_TMP2}")
        tran.writeAction(f"andi {REG_ALIAS_NWID_CNT} {REG_TMP4} {cls.LANES_MASK}")  # tmp4 => current lane offset in the UD
        tran.writeAction(f"muli {REG_TMP4} {REG_TMP7} {SHTArrayBucket.DESC_SIZE}")
        tran.writeAction(f"add {REG_TMP2} {REG_TMP7} {REG_TMP2}")
        # tmp3 -> tmp buffer start lm addr
        tran.writeAction(f"muli {OB_BUCKETS_PER_LANE} {REG_TMP3} {cls.LANES_PER_UD}")
        tran.writeAction(f"muli {REG_TMP3} {REG_TMP3} {SHTArrayBucket.DESC_SIZE}")
        tran.writeAction(f"add {REG_TMP3} {REG_TMP2} {REG_TMP3}")
        # tmp4 -> current tmp buffer lm addr
        tran.writeAction(f"muli {REG_TMP4} {REG_TMP4} {SHTArrayBucket.LM_BUF_SIZE}")
        ...
        tran.writeAction(f"mov_reg2reg {OB_DRAM_ALLOC_ADDR} {REG_CUR_BUCKET_DRAM_ADDR}")
        # init the bucket per lane counter
        tran.writeAction(f"mov_imm2reg {REG_LANE_BUCKET_CNT} {0}")

        # Prepare call args
        tran.writeAction(f"{f'{CLS_NAME}-{FN_NAME}-TR-LB-lane_init_next'}: add {OB_TMP_BUF_LM_OFFSET} {'X7'} {REG_TMP7}")
        tran.writeAction(f"move {REG_TMP2} {0}({REG_TMP7}) 0 8")  # desc_lm_addr
        ...
        tran.writeAction(f"move {REG_TMP4} {cls.WORD_SIZE * 3}({REG_TMP7}) 0 8")  # lm_buf_addr
        ...
        SHTArrayBucket.initialize(tran, tran_lane_init_ret.getLabel(), REG_TMP8, REG_TMP9, REG_TMP7, dst_nwid_reg=REG_ALIAS_NWID_CNT)
        ...
        tran.writeAction(f"andi {REG_ALIAS_NWID_CNT} {REG_TMP7} {cls.LANES_MASK}")
        tran.writeAction(f"addi {REG_TMP4} {REG_TMP4} {SHTArrayBucket.LM_BUF_SIZE}")
        tran.writeAction(f"bnei {REG_TMP7} {0} {f'{CLS_NAME}-{FN_NAME}-TR-LB-lane_init_next'}")

        # - next UD, reset counters
        tran.writeAction(f"mov_reg2reg {OB_BUCKET_DESC_LM_OFFSET} {REG_TMP2}")
    ...
    @classmethod
    def initialize(cls, tran: Transition, ret, tmp_reg0: str, tmp_reg1: str, arg_lm_addr_reg: str, dst_nwid_reg: str = 'X0') -> None:
        if type(ret) is int:
            sht_macros.call_wlm(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_init, ret_tran_label=ret, tmp_reg0=tmp_reg0, tmp_reg1=tmp_reg1, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=8)
        elif type(ret) is str:
            sht_macros.call_wlm_cont(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_init, cont_reg=ret, tmp_reg=tmp_reg0, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=8)
    ...
    @classmethod
    def finalize(cls, tran: Transition, ret, tmp_reg0: str, tmp_reg1: str, dst_nwid_reg: str = 'X0') -> None:
        if type(ret) is int:
            sht_macros.call_wreg(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_init, ret_tran_label=ret, tmp_reg0=tmp_reg0, tmp_reg1=tmp_reg1, arg_reg0='X0', arg_reg1='X0')
        elif type(ret) is str:
            sht_macros.call_wreg_cont(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_init, cont_reg=ret, tmp_reg=tmp_reg0, arg_reg0='X0', arg_reg1='X0')
    ...
    def _update(cls, state: State) -> None:
        ...
        tran.writeAction(f"mul {REG_TMP1} {REG_TMP2} {REG_TMP5}")  # tmp5 -> ud bucket offset
        ...
        tran.writeAction(f"add {REG_TMP5} {REG_TMP1} {REG_TMP1}")  # tmp1 -> ud bucket offset
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] UPDATE: ud bucket off = %d' {'X0'} {REG_TMP1}")
        tran.writeAction(f"muli {REG_TMP1} {REG_TMP1} {SHTArrayBucket.DESC_SIZE}")

        tran.writeAction(f"move {cls.DESC_STURCT_OFF_BUCKET_DESC_LM_ADDR}({REG_SHT_DESC_LM_ADDR}) {REG_TMP4} 0 8")  # tmp4 -> bucket desc lm addr base
        tran.writeAction(f"add {REG_TMP4} {REG_TMP1} {REG_TMP1}")  # tmp1 -> bucket desc lm addr

        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] UPDATE: bucket desc lm addr = %d' {'X0'} {REG_TMP1}")
        ...
        SHTArrayBucket.update(tran=tran, ret='X1', tmp_reg0=REG_TMP4, tmp_reg1=REG_TMP5, arg_lm_addr_reg=REG_TMP_BUF_LM_ADDR, arg_lm_words=3, dst_nwid_reg=REG_TMP3)
        tran.writeAction("yield_terminate")

    @classmethod
    def update(cls, tran: Transition, ret, tmp_reg0: str, tmp_reg1: str, arg_lm_addr_reg: str, dst_nwid_reg: str = 'X0') -> None:
        if type(ret) is int:
            sht_macros.call_wlm(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_update, ret_tran_label=ret, tmp_reg0=tmp_reg0, tmp_reg1=tmp_reg1, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=4)
        elif type(ret) is str:
            sht_macros.call_wlm_cont(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_update, cont_reg=ret, tmp_reg=tmp_reg0, arg_lm_addr_reg=arg_lm_addr_reg, arg_lm_words=4)
    ...
    def _get(cls, state: State) -> None:
        ...
        tran.writeAction(f"add {REG_TMP5} {REG_TMP1} {REG_TMP1}")  # tmp1 -> ud bucket offset
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] GET: ud bucket off = %d' {'X0'} {REG_TMP1}")
        tran.writeAction(f"muli {REG_TMP1} {REG_TMP1} {SHTArrayBucket.DESC_SIZE}")
        ...
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] GET: bucket desc lm addr = %d' {'X0'} {REG_TMP1}")

        # FIXME
        SHTArrayBucket.get(tran=tran, ret='X1', tmp_reg0=REG_TMP4, tmp_reg1=REG_TMP5, desc_lm_offset_reg=REG_TMP1, key_reg=OB_KEY, dst_nwid_reg=REG_TMP3)
        tran.writeAction("yield_terminate")

    @classmethod
    def get(cls, tran: Transition, ret, tmp_reg0: str, tmp_reg1: str, desc_lm_offset_reg: str, key_reg: str, dst_nwid_reg: str = 'X0'):
        if type(ret) is int:
            ...
            sht_macros.call_wreg_cont(tran=tran, dst_nwid_reg=dst_nwid_reg, callee_tran_label=cls.tran_label_get, cont_reg=ret, tmp_reg=tmp_reg0, arg_reg0=desc_lm_offset_reg, arg_reg1=key_reg)
