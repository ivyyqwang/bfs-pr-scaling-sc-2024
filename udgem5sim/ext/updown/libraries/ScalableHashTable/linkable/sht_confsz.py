...
@efaProgram
def SHTConfszModule(efa):
    efa.code_level = 'machine'

    state0 = efa.State()
    efa.add_initId(state0.state_id)

    SHTConfsz.setup(state0, debug=False)
...
class SHTConfsz:
    ...
    def _initialize(cls, state: EFAProgram.State):
        ...
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] INIT, num_alloc_lanes = %d, bucket_per_lane = %d, ent_per_bucket = %d, value_words = %d' {'X0'} {OB_NUM_ALLOC_LANES} {REG_BUCKETS_PER_LANE} {REG_ENTRIES_PER_BUCKET} {REG_VALUE_WORDS}")
        # Save continuation
        ...
        SHTArrayBucket.initialize(tran, tran_lane_init_ret.getLabel(), REG_TMP8, REG_TMP7, dst_nwid_reg=REG_ALIAS_NWID_CNT)
    ...
    def _update(cls, state: EFAProgram.State) -> None:
        ...
        if cls.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %d][{CLS_NAME}] UPDATE: number of val operands = %d' {'X0'} {REG_TMP0}")
