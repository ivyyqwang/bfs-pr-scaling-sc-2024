...
def hash(tran: EFAProgram.Transition, seed_reg: str, key_reg: str, num_entries_reg: str) -> None:
    """
...
def call_wreg(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, ret_tran_label: int, tmp_reg0: str, tmp_reg1: str, arg_reg0: str, arg_reg1: str, arg_reg2: str = "", branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg0} 255 {0b100}")  # setup a new thread
...
def call_wlm(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, ret_tran_label: int, tmp_reg0: str, tmp_reg1: str, arg_lm_addr_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg0} 255 {0b100}")  # setup a new thread
    # tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evii {tmp_reg0} 255 255 {0b100}")  # setup a new thread
    tran.writeAction(f"evlb {tmp_reg0} {callee_tran_label}")  # update callee event
    tran.writeAction(f"ev {tmp_reg0} {tmp_reg0} {dst_nwid_reg} {dst_nwid_reg} 8")  # update destination nwid
...
def call_wops(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, ret_tran_label: int, tmp_reg0: str, tmp_reg1: str, arg_start_op: str, arg_num_ops: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg0} 255 {0b100}")  # setup a new thread
    # tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evii {tmp_reg0} 255 255 {0b100}")  # setup a new thread
    tran.writeAction(f"evlb {tmp_reg0} {callee_tran_label}")  # update callee event
    tran.writeAction(f"ev {tmp_reg0} {tmp_reg0} {dst_nwid_reg} {dst_nwid_reg} 8")  # update destination nwid
    tran.writeAction(f"addi {'X2'} {tmp_reg1} {0}")
    tran.writeAction(f"evlb {tmp_reg1} {ret_tran_label}")  # update return event
    tran.writeAction(f"sendops {tmp_reg0} {tmp_reg1} {arg_start_op} {arg_num_ops} 0")  # generate call


def call_wreg_cont(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, cont_reg: str, tmp_reg: str, arg_reg0: str, arg_reg1: str, arg_reg2: str = "", branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg} 255 {0b100}")  # setup a new thread
...
def call_wlm_cont(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, cont_reg: str, tmp_reg: str, arg_lm_addr_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg} 255 {0b100}")  # setup a new thread
    # tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evii {tmp_reg} 255 255 {0b100}")  # setup a new thread
    tran.writeAction(f"evlb {tmp_reg} {callee_tran_label}")  # update callee event
    tran.writeAction(f"ev {tmp_reg} {tmp_reg} {dst_nwid_reg} {dst_nwid_reg} 8")  # update destination nwid
...
def call_wops_cont(tran: EFAProgram.Transition, dst_nwid_reg: str, callee_tran_label: int, cont_reg: str, tmp_reg: str, arg_start_op_reg: str, arg_num_ops: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evi {'X2'} {tmp_reg} 255 {0b100}")  # setup a new thread
    # tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}evii {tmp_reg} 255 255 {0b100}")  # setup a new thread
    tran.writeAction(f"evlb {tmp_reg} {callee_tran_label}")  # update callee event
    tran.writeAction(f"ev {tmp_reg} {tmp_reg} {dst_nwid_reg} {dst_nwid_reg} 8")  # update destination nwid
    tran.writeAction(f"sendops {tmp_reg} {cont_reg} {arg_start_op_reg} {arg_num_ops} 0")  # generate call


def return_wreg(tran: EFAProgram.Transition, cont_reg: str, tmp_reg: str, arg_reg0: str, arg_reg1: str, arg_reg2: str = "", branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}movir {tmp_reg} {-1}")
    tran.writeAction(f"sri {tmp_reg} {tmp_reg} {1}")
    if arg_reg2 == "":
        tran.writeAction(f"sendr {cont_reg} {tmp_reg} {arg_reg0} {arg_reg1}")
    else:
        tran.writeAction(f"sendr3 {cont_reg} {tmp_reg} {arg_reg0} {arg_reg1} {arg_reg2}")


def return_wlm(tran: EFAProgram.Transition, cont_reg: str, tmp_reg: str, arg_lm_addr_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}movir {tmp_reg} {-1}")
    tran.writeAction(f"sri {tmp_reg} {tmp_reg} {1}")
    tran.writeAction(f"send {cont_reg} {tmp_reg} {arg_lm_addr_reg} {arg_lm_words} 0")


def dram_write_reg_ret(tran: EFAProgram.Transition, addr_reg: str, ret_tran_label: int, tmp_reg: str, arg_reg0: str, arg_reg1: str = "", branch_label: str = "") -> None:
...
def dram_write_reg_cont(tran: EFAProgram.Transition, addr_reg: str, cont_reg: str, arg_reg0: str, arg_reg1: str = "", branch_label: str = "") -> None:
    if arg_reg1 == "":
...
def dram_write_lm_ret(tran: EFAProgram.Transition, addr_reg: str, ret_tran_label: int, tmp_reg: str, arg_lm_addr_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}addi {'X2'} {tmp_reg} {0}")
    tran.writeAction(f"evlb {tmp_reg} {ret_tran_label}")  # update return event
    tran.writeAction(f"sendm {addr_reg} {tmp_reg} {arg_lm_addr_reg} {arg_lm_words} {0b01}")


def dram_write_lm_cont(tran: EFAProgram.Transition, addr_reg: str, cont_reg: str, arg_lm_addr_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}sendm {addr_reg} {cont_reg} {arg_lm_addr_reg} {arg_lm_words} {0b01}")


def dram_read_ret(tran: EFAProgram.Transition, addr_reg: str, ret_tran_label: int, tmp_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
    tran.writeAction(f"{'' if branch_label == '' else branch_label + ': '}addi {'X2'} {tmp_reg} {0}")
    tran.writeAction(f"evlb {tmp_reg} {ret_tran_label}")  # update return event
    tran.writeAction(f"sendm {addr_reg} {tmp_reg} {tmp_reg} {arg_lm_words} {0b00}")  # WARN: invalid field for Xptr


def dram_read_cont(tran: EFAProgram.Transition, addr_reg: str, cont_reg: str, arg_lm_words: int, branch_label: str = "") -> None:
...
# def hashcall_wreg(tran: EFAProgram.Transition, key_reg: str, start_nwid_reg: str, num_lanes_reg: str, seed_reg: str, callee_tran_label: int, ret_tran_label: int, tmp_reg0: str, arg_reg0: str, arg_reg1: str, arg_reg2: str = "") -> None:
#     """
...
# def hashcall_wlm(tran: EFAProgram.Transition, key_reg: str, start_nwid_reg: str, num_lanes_reg: str, seed_reg: str, callee_tran_label: int, ret_tran_label: int, tmp_reg0: str, arg_lm_addr_reg: str, arg_lm_len: int) -> None:
#     """
#     Hash the specified key to the specified UpDown lane range, create a new thread on the hash mapped network ID, and trigger the specified transition.
...
# def return_wreg(tran: EFAProgram.Transition, saved_cont_reg: str, arg_reg0: str, arg_reg1: str, arg_reg2: str = "") -> None:
#     tran.writeAction(f"sendr_wcont {saved_cont_reg} {saved_cont_reg} {arg_reg0} {arg_reg1} {arg_reg2}")  # WARN: the continuation is invalid


# def return_wlm(tran: EFAProgram.Transition, saved_cont_reg: str, arg_lm_addr_reg: str, arg_lm_words: int) -> None:
...
class SpinLock:
    ...
    def init(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
        """
        tran.writeAction(f"mov_imm2reg {tmp_reg} {0}")
        tran.writeAction(f"move {tmp_reg} {0}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")

    def acquire(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str, num_ops: int) -> None:
        """
        """
        if not self.ATOMIC:
            tran.writeAction(f"move {0}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")
            tran.writeAction(f"beqiu {tmp_reg} {0} {f'LB-spin_lock-{self.ID}-acquired'}")
            tran.writeAction(f"evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
            tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {num_ops}")  # delay current event, send to self with current continuation and all operands
            tran.writeAction("yield_terminate")
            tran.writeAction(f"{f'LB-spin_lock-{self.ID}-acquired'}: addi {tmp_reg} {tmp_reg} 1")  # increment lock
            tran.writeAction(f"move {tmp_reg} {0}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")

    def release(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
...
#     def init(self, tran: EFAProgram.Transition, tmp_reg: str) -> None:
#         """
#         """
#         tran.writeAction(f"mov_imm2reg {tmp_reg} {0}")
#         tran.writeAction(f"move {tmp_reg} {self.LOCK_LM_ADDR}(X7) 0 {self.LOCK_SIZE}")

#     def acquire(self, tran: EFAProgram.Transition, tmp_reg: str, num_ops: int) -> None:
#         """
#         """
#         if not self.ATOMIC:
...
#     def release(self, tran: EFAProgram.Transition, tmp_reg: str) -> None:
#         """
#         should not be called when not holding a lock
#         """
#         if not self.ATOMIC:
...
class MRSWLock:
    ...
    def init(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        ...
        if self.DEBUG:
            tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] INIT' {'X0'} {lock_lm_addr_reg}")
        self.CALL_CNT += 1

    def read_begin(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str, num_ops: int) -> None:
        """
        """
        ...
        if not self.ATOMIC:
            ...
            tran.writeAction(f"beqiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-reader_acquired'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: reader lock busy, delay current event' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
            tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {num_ops}")  # delay current event, send to self with current continuation and all operands
            tran.writeAction("yield_terminate")
            ...
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: reader lock acquired, reader counter = %lu' {'X0'} {lock_lm_addr_reg} {tmp_reg}")
            tran.writeAction(f"bgtiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-inc_release_reader'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: first active reader, checking writer lock...' {'X0'} {lock_lm_addr_reg} {lock_lm_addr_reg}")
            tran.writeAction(f"move {self.LOCK_SIZE}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")  # get writer lock
            tran.writeAction(f"bgtiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-release_reader_spin'}")  # writer is locked, spin
            # acquire writer lock
            tran.writeAction(f"addi {tmp_reg} {tmp_reg} 1")  # increment lock
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: writer lock free, writer lock acquired' {'X0'}")
            ...
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: writer lock busy, free reader lock & delay current event' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"subi {tmp_reg} {tmp_reg} 1")  # decrement lock
            tran.writeAction(f"move {tmp_reg} {0}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            tran.writeAction(f"evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
            tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {num_ops}")  # delay current event, send to self with current continuation and all operands
            ...
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: read permitted, increase reader counter & release reader lock' {'X0'} {lock_lm_addr_reg}")
    ...
    def read_begin_var(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
        """
        MACRO_NAME = "read_begin"
        if not self.ATOMIC:
            ...
            tran.writeAction(f"rshift {'X2'} {tmp_reg} {20}")
            tran.writeAction(f"andi {tmp_reg} {tmp_reg} {0b111}")
            for i in range(0, 7):
                tran.writeAction(f"beqiu {tmp_reg} {i} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-reader_wait_{i + 2}'}")
            tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: SHOULDN'T REACH HERE!!!!' {'X0'} {lock_lm_addr_reg}")
            ...
            tran.writeAction(f"move {self.LOCK_SIZE * 2}({lock_lm_addr_reg}) {tmp_reg} 0 {self.READER_COUNTER_SIZE}")  # get reader counter
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: reader lock acquired, reader counter = %d' {'X0'} {lock_lm_addr_reg} {tmp_reg}")
            tran.writeAction(f"bgtiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-inc_release_reader'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: first active reader, checking writer lock...' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"move {self.LOCK_SIZE}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")  # get writer lock
            tran.writeAction(f"bgtiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-release_reader_spin'}")  # writer is locked, spin
            # acquire writer lock
            tran.writeAction(f"addi {tmp_reg} {tmp_reg} 1")  # increment lock
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: writer lock free, writer lock acquired' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"jmp {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-inc_release_reader'}")
            # release reader lock & spin
            tran.writeAction(f"{f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-release_reader_spin'}: move {0}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")
            if self.DEBUG:
            ...
            for i in range(0, 7):
                tran.writeAction(f"{f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-reader_wait_{i + 2}'}: evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
                tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {i + 2}")  # send to self with current continuation and all operands
                if self.DEBUG:
                    tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_BEGIN: reader lock busy, delay current event ({i + 2} operands)' {'X0'} {lock_lm_addr_reg}")
                tran.writeAction("yield_terminate")
        self.CALL_CNT += 1

    def read_end(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
        """
        MACRO_NAME = "read_end"
        if not self.ATOMIC:
            ...
            tran.writeAction(f"beqiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-reader_acquired'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_END: reader lock busy, loop spin wait (to be freed from another lane)' {'X0'} {lock_lm_addr_reg}")
            ...
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_END: reader lock acquired, reader counter = %d' {'X0'} {lock_lm_addr_reg} {tmp_reg}")
            tran.writeAction(f"bgtiu {tmp_reg} {1} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-dec_release_reader'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_END: last active reader, releasing writer lock' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"move {self.LOCK_SIZE}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")
            tran.writeAction(f"beqiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-dec_release_reader'}")  # writer not locked??
            # release writer lock
            tran.writeAction(f"subi {tmp_reg} {tmp_reg} 1")  # decrement lock
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            # increment reader counter & release reader lock
            ...
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] READ_END: read finished, decrement reader counter & release reader lock' {'X0'} {lock_lm_addr_reg}")
    ...
    def write_begin(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str, num_ops: int) -> None:
        """
        """
        MACRO_NAME = "write_begin"
        if not self.ATOMIC:
            tran.writeAction(f"move {self.LOCK_SIZE}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")
            tran.writeAction(f"beqiu {tmp_reg} {0} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-acquired'}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] WRITE_BEGIN: writer lock busy, delay current event' {'X0'} {lock_lm_addr_reg}")
            tran.writeAction(f"evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
            tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {num_ops}")  # delay current event, send to self with current continuation and all operands
            tran.writeAction("yield_terminate")
            ...
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] WRITE_BEGIN: writer lock free, acquire writer lock' {'X0'} {lock_lm_addr_reg}")
        self.CALL_CNT += 1

    def write_begin_var(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
        """
        MACRO_NAME = "write_begin"
        if not self.ATOMIC:
            ...
            tran.writeAction(f"andi {tmp_reg} {tmp_reg} {0b111}")
            for i in range(0, 7):
                tran.writeAction(f"beqiu {tmp_reg} {i} {f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-writer_wait_{i + 2}'}")
            tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] WRITE_BEGIN: SHOULDN'T REACH HERE!!!!' {'X0'} {lock_lm_addr_reg}")
            for i in range(0, 7):
                tran.writeAction(f"{f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-writer_wait_{i + 2}'}: evi {'X2'} {tmp_reg} 255 {0b100}")  # new thread
                tran.writeAction(f"sendops_wcont {tmp_reg} {'X1'} {'X8'} {i + 2}")  # send to self with current continuation and all operands
                if self.DEBUG:
                    tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] WRITE_BEGIN: writer lock busy, delay current event ({i + 2} operands)' {'X0'} {lock_lm_addr_reg}")
                tran.writeAction("yield_terminate")
            tran.writeAction(f"{f'LB-mrsw_lock-{self.ID}-{MACRO_NAME}-{self.CALL_CNT}-acquired'}: addi {tmp_reg} {tmp_reg} 1")  # increment lock
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            if self.DEBUG:
    ...
    def write_end(self, tran: EFAProgram.Transition, lock_lm_addr_reg: str, tmp_reg: str) -> None:
        """
        """
        if not self.ATOMIC:
            tran.writeAction(f"move {self.LOCK_SIZE}({lock_lm_addr_reg}) {tmp_reg} 0 {self.LOCK_SIZE}")
            ...
            tran.writeAction(f"move {tmp_reg} {self.LOCK_SIZE}({lock_lm_addr_reg}) 0 {self.LOCK_SIZE}")
            if self.DEBUG:
                tran.writeAction(f"print '[DEBUG][NWID %lu][MRSWLock {self.ID}][%p] WRITE_END: writer lock freed' {'X0'} {lock_lm_addr_reg}")
        self.CALL_CNT += 1
...
#     def read_begin(self, tran: EFAProgram.Transition, tmp_reg: str, num_ops: int) -> None:
#         """
#         """
#         MACRO_NAME = "read_begin"
#         if not self.ATOMIC:
...
#     def read_end(self, tran: EFAProgram.Transition, tmp_reg: str) -> None:
#         """
#         """
#         MACRO_NAME = "read_end"
#         if not self.ATOMIC:
...
#     def write_begin(self, tran: EFAProgram.Transition, tmp_reg: str, num_ops: int) -> None:
#         """
#         """
#         MACRO_NAME = "write_begin"
#         if not self.ATOMIC:
...
#     def write_end(self, tran: EFAProgram.Transition, tmp_reg: str) -> None:
#         """
#         """
#         if not self.ATOMIC:
#             tran.writeAction(f"move {self.WRITER_LOCK_LM_ADDR}(X7) {tmp_reg} 0 {self.LOCK_SIZE}")
