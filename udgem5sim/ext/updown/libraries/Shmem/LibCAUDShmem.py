...
@efaProgram
def LibCAUDShmem(efa: EFAProgram):
    """
    ...
    shmem = CAUDShmem(efa, state0, event_map_start=0, debug_level=2, perflog_level=5, term_flag_offset=0, impl=CAUDShmem.implLocAwareInterleaving, linker=True)# init generate EFAs, efa and states are required for now, since we have block actions
    return efa