...
@efaProgram
def SpMalloc(efa: EFAProgram):
    efa.code_level = 'machine'

    state0 = efa.State()
    efa.add_initId(state0.state_id)

    spmalloc_inst = SpMalloc(LMStaticMap.HEAP_OFFSET, "lm_allocator")
...
class SpMallocImpl:
    def __init__(self, init_offset, bank_size=65536, word_width=8, registers=None):
...
#            f"init: lshift_add_imm {lane_id} {bank_end_address} {self.shift_size} {self.bank_size}")
        ...
        tran.writeAction(
            f"addi {bank_start_address} {bank_first_heap_address} {self.init_offset}")
    ...
    def allocate_block(self, tran):
        ...
        tran.writeAction(f"blti {remaining} {self.block_break_threshold} post_allocation_header_assignment")
...
class SpMalloc:
    ...
    def __init__(self, init_offset, thread_name, bank_size=65536, word_width=8, registers=None):
        """
        Parameters
        ...
        self.sp_malloc_writer = SpMallocImpl(
            init_offset, bank_size, word_width, registers)
