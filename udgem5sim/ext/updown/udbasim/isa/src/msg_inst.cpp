...
Cycles exeInstSendm(ArchState& ast, EncInst inst) {
  ...
  if (!physical_addr) {
    BASIM_ERROR("Translation failed for address %lu on nwid %d tid %d, INSTR=%u Sendm dest_reg=X%d cont_reg=X%d", \
                tst->readReg(extrInstSendmXd(inst)), tst->getNWIDbits(), tst->getTID(), inst, (int) extrInstSendmXd(inst), (int) extrInstSendmXc(inst));
    exit(1);
  }
...
Cycles exeInstSendops(ArchState& ast, EncInst inst) {
  ...
  ev.setNumOperands(len - 2);
  if (!ast.transmem->validate_nwid(ev.getNWID())) {
    BASIM_ERROR("Translation failed for nwid %d on nwid %d tid %d, INSTR=%u SENDOPS dest_ev_reg=X%d data_reg=X%d cont_reg=X%d", \
