from EFA_v2 import *
def basic_noaction1_with_lastact_TX_4():
    efa = EFA([])
    efa.code_level = "machine"
    state = State()
    efa.add_initId(state.state_id)
    efa.add_state(state)
    state1 = State()
    state1.alphabet = [0-255]
    efa.add_state(state1)
    state2 = State()
    state2.alphabet = [0-255]
    efa.add_state(state2)
    tran0 = state.writeTransition("basic_with_action", state, state1, 1)
    tran0.writeAction("movir X17 0")
    tran0.writeAction("add X7 X17 X17")
    tran0.writeAction("movir X16 29357")
    tran0.writeAction("slorii X16 X16 12 3062")
    tran0.writeAction("slorii X16 X16 12 439")
    tran0.writeAction("slorii X16 X16 12 1946")
    tran0.writeAction("slorii X16 X16 12 1862")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 47198")
    tran0.writeAction("slorii X16 X16 12 241")
    tran0.writeAction("slorii X16 X16 12 3866")
    tran0.writeAction("slorii X16 X16 12 2468")
    tran0.writeAction("slorii X16 X16 12 3867")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 40683")
    tran0.writeAction("slorii X16 X16 12 2566")
    tran0.writeAction("slorii X16 X16 12 1095")
    tran0.writeAction("slorii X16 X16 12 599")
    tran0.writeAction("slorii X16 X16 12 1757")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 10977")
    tran0.writeAction("slorii X16 X16 12 873")
    tran0.writeAction("slorii X16 X16 12 3249")
    tran0.writeAction("slorii X16 X16 12 3463")
    tran0.writeAction("slorii X16 X16 12 2512")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 57374")
    tran0.writeAction("slorii X16 X16 12 3911")
    tran0.writeAction("slorii X16 X16 12 932")
    tran0.writeAction("slorii X16 X16 12 2495")
    tran0.writeAction("slorii X16 X16 12 2165")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 20388")
    tran0.writeAction("slorii X16 X16 12 1017")
    tran0.writeAction("slorii X16 X16 12 2584")
    tran0.writeAction("slorii X16 X16 12 120")
    tran0.writeAction("slorii X16 X16 12 2456")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 58489")
    tran0.writeAction("slorii X16 X16 12 1683")
    tran0.writeAction("slorii X16 X16 12 1768")
    tran0.writeAction("slorii X16 X16 12 1118")
    tran0.writeAction("slorii X16 X16 12 2660")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 10999")
    tran0.writeAction("slorii X16 X16 12 595")
    tran0.writeAction("slorii X16 X16 12 4055")
    tran0.writeAction("slorii X16 X16 12 1352")
    tran0.writeAction("slorii X16 X16 12 3368")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X18 1")
    tran0.writeAction("sli X18 X18 23")
    tran0.writeAction("or X18 X2 X2")
    tran0.writeAction("movir X16 29357")
    tran0.writeAction("slorii X16 X16 12 3062")
    tran0.writeAction("slorii X16 X16 12 439")
    tran0.writeAction("slorii X16 X16 12 1946")
    tran0.writeAction("slorii X16 X16 12 1862")
    tran0.writeAction("sri X16 X16 41")
    tran0.writeAction("sli X16 X16 41")
    tran0.writeAction("movir X17 1")
    tran0.writeAction("sli X17 X17 40")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X17 5")
    tran0.writeAction("sli X17 X17 36")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X17 7")
    tran0.writeAction("sli X17 X17 32")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X31 0")
    tran0.writeAction("add X7 X31 X31")
    tran0.writeAction("movir X20 0")
    tran0.writeAction("sli X31 X21 3")
    tran0.writeAction("add X21 X20 X5")
    tran0.writeAction("movlsb X31")
    tran0.writeAction("addi X20 X17 21")
    tran0.writeAction("or X17 X16 X4")
    tran0.writeAction("addi X4 X18 0")
    tran0.writeAction("movir X30 0")
    tran0.writeAction("movir X31 0")
    tran0.writeAction("lastact")
    tran1 = state1.writeTransition("basic", state1, state2, 6)
    tran3 = state1.writeTransition("basic_with_action", state1, state2, 255)
    tran3.writeAction("movir X30 8")
    tran3.writeAction("lastact")
    tran2 = state2.writeTransition("basic_with_action", state2, state2, 14)
    tran2.writeAction("movir X31 7")
    tran2.writeAction("yieldt")
    tran4 = state2.writeTransition("basic_with_action", state2, state1, 255)
    tran4.writeAction("addi X5 X17 0")
    tran4.writeAction("movir X31 8")
    tran4.writeAction("yieldt")
    efa.appendBlockAction("end_of_input_terminate_efa1","movir X29 10")
    efa.appendBlockAction("end_of_input_terminate_efa1","yieldt")
    efa.linkBlocktoState("end_of_input_terminate_efa1", state2)
    return efa
