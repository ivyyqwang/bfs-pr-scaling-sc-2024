from EFA_v2 import *
def basic_noaction1_with_lastact_TX_maxSBP_3():
    efa = EFA([])
    efa.code_level = "machine"
    state = State()
    efa.add_initId(state.state_id)
    efa.add_state(state)
    state1 = State()
    state1.alphabet = [0-255]
    efa.add_state(state1)
    state2 = State()
    state2.alphabet = [0-255]
    efa.add_state(state2)
    tran0 = state.writeTransition("basic_with_action", state, state1, 1)
    tran0.writeAction("movir X17 0")
    tran0.writeAction("add X7 X17 X17")
    tran0.writeAction("movir X16 59598")
    tran0.writeAction("slorii X16 X16 12 1084")
    tran0.writeAction("slorii X16 X16 12 579")
    tran0.writeAction("slorii X16 X16 12 1959")
    tran0.writeAction("slorii X16 X16 12 328")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 5722")
    tran0.writeAction("slorii X16 X16 12 1592")
    tran0.writeAction("slorii X16 X16 12 2805")
    tran0.writeAction("slorii X16 X16 12 2149")
    tran0.writeAction("slorii X16 X16 12 3471")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 11964")
    tran0.writeAction("slorii X16 X16 12 1459")
    tran0.writeAction("slorii X16 X16 12 2891")
    tran0.writeAction("slorii X16 X16 12 3665")
    tran0.writeAction("slorii X16 X16 12 1527")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 21877")
    tran0.writeAction("slorii X16 X16 12 344")
    tran0.writeAction("slorii X16 X16 12 2698")
    tran0.writeAction("slorii X16 X16 12 3120")
    tran0.writeAction("slorii X16 X16 12 1847")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 21564")
    tran0.writeAction("slorii X16 X16 12 1491")
    tran0.writeAction("slorii X16 X16 12 3650")
    tran0.writeAction("slorii X16 X16 12 134")
    tran0.writeAction("slorii X16 X16 12 1017")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 47590")
    tran0.writeAction("slorii X16 X16 12 653")
    tran0.writeAction("slorii X16 X16 12 303")
    tran0.writeAction("slorii X16 X16 12 1669")
    tran0.writeAction("slorii X16 X16 12 1591")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 852")
    tran0.writeAction("slorii X16 X16 12 1804")
    tran0.writeAction("slorii X16 X16 12 898")
    tran0.writeAction("slorii X16 X16 12 874")
    tran0.writeAction("slorii X16 X16 12 3623")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X16 60516")
    tran0.writeAction("slorii X16 X16 12 4035")
    tran0.writeAction("slorii X16 X16 12 1071")
    tran0.writeAction("slorii X16 X16 12 1973")
    tran0.writeAction("slorii X16 X16 12 2521")
    tran0.writeAction("movrl X16 0(X17) 1 8")
    tran0.writeAction("movir X18 1")
    tran0.writeAction("sli X18 X18 23")
    tran0.writeAction("or X18 X2 X2")
    tran0.writeAction("movir X16 59598")
    tran0.writeAction("slorii X16 X16 12 1084")
    tran0.writeAction("slorii X16 X16 12 579")
    tran0.writeAction("slorii X16 X16 12 1959")
    tran0.writeAction("slorii X16 X16 12 328")
    tran0.writeAction("sri X16 X16 41")
    tran0.writeAction("sli X16 X16 41")
    tran0.writeAction("movir X17 1")
    tran0.writeAction("sli X17 X17 40")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X17 8")
    tran0.writeAction("sli X17 X17 36")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X17 8")
    tran0.writeAction("sli X17 X17 32")
    tran0.writeAction("or X17 X16 X16")
    tran0.writeAction("movir X31 0")
    tran0.writeAction("add X7 X31 X31")
    tran0.writeAction("movir X20 0")
    tran0.writeAction("sli X31 X21 3")
    tran0.writeAction("add X21 X20 X5")
    tran0.writeAction("movlsb X31")
    tran0.writeAction("addi X20 X17 8")
    tran0.writeAction("or X17 X16 X4")
    tran0.writeAction("addi X4 X18 0")
    tran0.writeAction("movir X30 0")
    tran0.writeAction("movir X31 0")
    tran0.writeAction("lastact")
    tran1 = state1.writeTransition("basic", state1, state2, 72)
    tran3 = state1.writeTransition("basic_with_action", state1, state2, 255)
    tran3.writeAction("movir X30 8")
    tran3.writeAction("lastact")
    tran2 = state2.writeTransition("basic", state2, state2, 113)
    tran4 = state2.writeTransition("basic_with_action", state2, state1, 255)
    tran4.writeAction("addi X5 X17 0")
    tran4.writeAction("movir X31 8")
    tran4.writeAction("yieldt")
    efa.appendBlockAction("end_of_input_terminate_efa1","movir X29 10")
    efa.appendBlockAction("end_of_input_terminate_efa1","yieldt")
    efa.linkBlocktoState("end_of_input_terminate_efa1", state2)
    return efa
