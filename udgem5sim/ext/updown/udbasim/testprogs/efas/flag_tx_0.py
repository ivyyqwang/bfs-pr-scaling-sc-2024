...
def flag_tx_0():
    ...
    tran = state.writeTransition("flagCarry_with_action", state, state0, 1)
    ...
    #set X16 for flag tx
    tran.writeAction("movir X16 1")   #(mov_imm2reg) 
    tran.writeAction("lastact")

    tran = state0.writeTransition("flagCarry_with_action", state0, state0, 0)
    tran.writeAction("movir X17 0")   #(mov_imm2reg) 
    tran.writeAction("yieldt")
    tran = state0.writeTransition("flagCarry_with_action", state0, state1, 1)
    tran.writeAction("movir X17 1")   #(mov_imm2reg) 
    tran.writeAction("yieldt")
