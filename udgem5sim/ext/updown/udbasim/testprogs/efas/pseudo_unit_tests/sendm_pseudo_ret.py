...
def sendm_pseudo_ret():
    ...
    tran0.writeAction("addi X18 X20 8")  
    tran0.writeAction("movrl X17 0(X20) 0 8")  
    tran0.writeAction(f"send_dmlm_wret X8 {str(event_map['write_ret'])} X20 1 X21")
    tran0.writeAction("yield")

    tran1 = state0.writeTransition("eventCarry", state0, state0, event_map["read_ret"])
    tran1.writeAction("movrl X8 0(X20) 0 8")
    ...
    tran2.writeAction(f"send_dmlm_ld_wret X8 {str(event_map['read_ret'])} 1 X21")
