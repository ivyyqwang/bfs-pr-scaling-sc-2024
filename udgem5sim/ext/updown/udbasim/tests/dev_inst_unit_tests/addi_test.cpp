...
class Addi : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Addi, Random) {
  ...
  inst = constrInstAddi(RegId::X16, RegId::X17, v1 & 0x000000000000FFFF);
  ...
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X17), v2 + v1);
...
TEST_F(Addi, UnsignedLowerBound) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
TEST_F(Addi, UnsignedUpperBound) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
