...
TEST_F(Bcpyll, Random) {
  ...
  inst = constrInstBcpyll(RegId::X16, RegId::X17, RegId::X18);
  ...
  EXPECT_EQ(thread0->readReg(RegId::X18), 0);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

// /* Testing with invalid imm */
// TEST_F(Bcpyll, InvalidImm) {
//   ThreadStatePtr thread0;
//   uint8_t tid0 = tstable->getTID();
//   thread0 = new ThreadState(tid0, 0, 0, 0);
//   tstable->addtoTST(thread0);
...
//   // std::cout << "Cycles:" << cycle << std::endl;
//   //  read_reg() == reg + imm
//   EXPECT_EXIT(decodeInst(inst).exe(*archstate, inst), ::testing::ExitedWithCode(2 && "Invalid imm value in bcpyll"), ".*");
//   // Expect to be empty.
// }

/* Testing with low imm */
TEST_F(Bcpyll, LowImm) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
