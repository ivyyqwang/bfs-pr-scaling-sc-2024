...
class Beqiu : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Beqiu, Random) {
  ...
  uint64_t v1 = rnd();
  uint64_t v2 = static_cast<int64_t>((rnd() & 0x1F));
  ...
  // EncInst constrInstBneiuBeqiuBgtiuBleiuBltiuBgeiu(uint64_t func, RegId X1, int64_t imm, uint64_t targeta, int64_t targetb);
  inst = constrInstBneiuBeqiuBgtiuBleiuBltiuBgeiu(1, RegId::X16, v2, target & BF_BEQIU_TARGETA_MASK, (target >> BF_BEQIU_TARGETA_NBITS) & BF_BEQIU_TARGETB_MASK);
...
TEST_F(Beqiu, Taken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  uint64_t v1 = 0b11111;
  uint64_t v2 = static_cast<int64_t>((0b11111 & 0x1F));
...
TEST_F(Beqiu, NotTaken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  uint64_t v1 = 14;
  uint64_t v2 = static_cast<int64_t>((12 & 0x1F));
