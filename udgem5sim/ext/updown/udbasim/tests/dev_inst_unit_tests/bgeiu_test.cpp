...
class Bgeiu : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Bgeiu, Random) {
  ...
  inst = constrInstBneiuBeqiuBgtiuBleiuBltiuBgeiu(5, RegId::X16, v2, target & BF_BGEIU_TARGETA_MASK, (target >> BF_BGEIU_TARGETA_NBITS) & BF_BGEIU_TARGETB_MASK);
...
TEST_F(Bgeiu, Taken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  uint64_t v1 = 30;
  uint64_t v2 = static_cast<int64_t>((25 & 0x1F));
