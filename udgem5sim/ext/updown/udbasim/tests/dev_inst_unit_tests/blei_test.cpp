...
class Blei : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Blei, Random) {
  ...
  inst = constrInstBneiBeqiBgtiBleiBltiBgei(3, RegId::X16, v2, target & BF_BLEI_TARGETA_MASK, (target >> BF_BLEI_TARGETA_NBITS) & BF_BLEI_TARGETB_MASK);
...
TEST_F(Blei, Taken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  int64_t v1 = 12;
  int64_t v2 = static_cast<int64_t>((14 & 0x1F));
...
TEST_F(Blei, NotTaken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  int64_t v1 = 12;
  int64_t v2 = static_cast<int64_t>((6 & 0x1F));
