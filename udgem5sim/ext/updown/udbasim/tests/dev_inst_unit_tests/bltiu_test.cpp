...
class Bltiu : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Bltiu, Random) {
  ...
  inst = constrInstBneiuBeqiuBgtiuBleiuBltiuBgeiu(4, RegId::X16, v2, target & BF_BLTIU_TARGETA_MASK, (target >> BF_BLTIU_TARGETA_NBITS) & BF_BLTIU_TARGETB_MASK);
...
TEST_F(Bltiu, Taken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  uint64_t v1 = 12;
  uint64_t v2 = static_cast<int64_t>((14 & 0x1F));
...
TEST_F(Bltiu, NotTaken) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  uint64_t v1 = 12;
  uint64_t v2 = static_cast<int64_t>((6 & 0x1F));
