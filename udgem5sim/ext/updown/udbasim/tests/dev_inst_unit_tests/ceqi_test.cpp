...
class Ceqi : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Ceqi, Random) {
  ...
  uint64_t v1 = rnd();
  uint64_t v2 = static_cast<uint64_t>((static_cast<int64_t>(rnd() << 48)) >> 48);
  uint64_t v3 = rnd();

  inst = constrInstCeqi(RegId::X16, RegId::X17, v2 & 0x000000000000FFFF);
  ...
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X17), static_cast<int64_t>(v1) == static_cast<int64_t>(v2) ? 1 : 0);
...
TEST_F(Ceqi, LessThan) {
  ...
  uint64_t v2 = static_cast<uint64_t>((static_cast<int64_t>((rnd() << 48) & 0x7FFFFFFFFFFFFFFF)) >> 48);
  uint64_t v3 = rnd();
...
TEST_F(Ceqi, GreaterThan) {
  ...
  uint64_t v2 = static_cast<uint64_t>((static_cast<int64_t>((rnd() << 48) | 0x8000000000000000)) >> 48);
  uint64_t v3 = rnd();
