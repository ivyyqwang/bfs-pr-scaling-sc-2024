...
TEST_F(Div, Random) {
  ...
  inst = constrInstAddSubMulDivMod(3, RegId::X16, RegId::X17, RegId::X18);
...
TEST_F(Div, By0) {
  ...
  // std::cout << "Cycles:" << cycle << std::endl;
  // read_reg() == reg + imm
  EXPECT_EXIT(decodeInst(inst).exe(*archstate, inst), ::testing::ExitedWithCode(2 && "Divide by zero"), ".*");
  // Expect to be empty.
}

/* Testing pos divides pos */
TEST_F(Div, PosPos) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
