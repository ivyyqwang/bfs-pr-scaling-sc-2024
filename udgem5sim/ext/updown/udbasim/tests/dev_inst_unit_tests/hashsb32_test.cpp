...
class Hashsb32 : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
/* Vanila implementation of CRC64-DNP */
uint64_t crc16_helper_hashsb32(uint64_t input) {
...
/* Testing with sb random value */
TEST_F(Hashsb32, SbRandom) {
  ...
  v1 = v1 > (512 - 64) ? (512 - 64) : v1;
  uint64_t v2 = rnd() >> 48;
  uint64_t v3 = rnd();

  inst = constrInstHashsb32(RegId::X16, v2);
  thread0->writeReg(RegId::X2, thread0->readReg(RegId::X2) | 0x0000000000800000);
  ...
  EXPECT_EQ(thread0->readReg(RegId::X16), 2*crc16_helper_hashsb32((stbuff->getVarSymbol(v1, 8) << 0) | (stbuff->getVarSymbol(v1+8, 8) << 8) | (stbuff->getVarSymbol(v1+16, 8) << 16) | (stbuff->getVarSymbol(v1+24, 8) << 24)) + v2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing with sb max value */
TEST_F(Hashsb32, SbMax) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  ...
  EXPECT_EQ(thread0->readReg(RegId::X16), 2*crc16_helper_hashsb32(0xFFFFFFFF) + v2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}


/* Testing with sb min value */
TEST_F(Hashsb32, SbMin) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  ...
  EXPECT_EQ(thread0->readReg(RegId::X16), 2*crc16_helper_hashsb32(0x00000000) + v2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing with lm random value */
TEST_F(Hashsb32, LmRandom) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  ...
  inst = constrInstHashsb32(RegId::X16, v2);
  thread0->writeReg(RegId::X2, thread0->readReg(RegId::X2) & 0xFFFFFFFFFF7FFFFF);
  thread0->writeReg(RegId::X5, v1);
  spd->writeBytes(4, v1, reinterpret_cast<uint8_t *>(&v5));
  ...
  EXPECT_EQ(thread0->readReg(RegId::X5), v1);
  uint8_t tmp[4];
  spd->readBytes(4, v1, tmp);
  EXPECT_EQ(*(reinterpret_cast<uint32_t *>(tmp)), v5);
  EXPECT_EQ(thread0->readReg(RegId::X16), crc16_helper_hashsb32(v5) + v2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing with lm max value */
TEST_F(Hashsb32, LmMax) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
/* Testing with lm min value */
TEST_F(Hashsb32, LmMin) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
