...
TEST_F(Movil2, Random) {
  ...
  uint64_t v1 = ((rnd() << 48) >> 49) << 1;
  v1 = v1 > (65536 - 2) ? (65536 - 2) : v1;
  uint64_t v2 = (v1 >> 3) << 3;
  uint64_t v3 = rnd() >> 48;
  uint64_t v4 = rnd();
  ...
  inst = constrInstMovil2(RegId::X16, v3 & 0x000000000000FFFF);
  thread0->writeReg(RegId::X16, v1);
  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  //  read_reg() == reg + imm
#ifdef LITTLEENDIAN
  EXPECT_EQ(spd->readWord(v2), (v4&(~(static_cast<uint64_t>(0x000000000000FFFF)<<(8*(v1-v2))))) | ((v3&0x000000000000FFFF)<<(8*(v1-v2))));
#else
  EXPECT_EQ(spd->readWord(v2), (v4&(~(0xFFFF000000000000>>(8*(v1-v2))))) | ((v3&0x000000000000FFFF)<<(48-8*(v1-v2))));
#endif
  EXPECT_EQ(thread0->readReg(RegId::X16), v1+2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing with out of bound imm */
TEST_F(Movil2, OutofBoundImm) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  ...
  inst = constrInstMovil2(RegId::X16, v3);
  thread0->writeReg(RegId::X16, v1);
  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  //  read_reg() == reg + imm
