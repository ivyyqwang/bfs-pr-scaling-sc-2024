...
class Movipr : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Movipr, Random) {
  ...
  inst = constrInstMovipr(RegId::X16);
  thread0->writeReg(RegId::X16, v1);
  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X16) + 4, archstate->uip);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing with random times */
TEST_F(Movipr, RandomTimes) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  archstate->threadstate = thread0;
  Cycles cycle = basim::Cycles(0);
  ...
  for (auto i = 0; i < v1; i++) {
    inst = constrInstMovipr(RegId::X16);
    thread0->writeReg(RegId::X16, rnd());
