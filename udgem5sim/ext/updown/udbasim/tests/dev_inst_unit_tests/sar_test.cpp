...
TEST_F(Sar, Random) {
  ...
  inst = constrInstSrSlSar(2, RegId::X16, RegId::X17, RegId::X18);
  ...
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X18), v2 >= 64 ? ((v1 & 0x8000000000000000) ? 0xFFFFFFFFFFFFFFFF : 0) : static_cast<uint64_t>(static_cast<int64_t>(v1) >> v2));
