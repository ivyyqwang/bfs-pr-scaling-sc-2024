...
TEST_F(Sari, Random) {
  ...
  inst = constrInstSari(RegId::X16, RegId::X17, v2 & 0x000000000000FFFF);
  ...
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X17), static_cast<uint64_t>(static_cast<int64_t>(v1) >> (v2 & 0x000000000000FFFF)));
