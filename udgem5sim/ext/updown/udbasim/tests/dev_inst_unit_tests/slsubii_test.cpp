...
class Slsubii : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Slsubii, RandomLeft) {
  ...
  inst = constrInstSlsubii(RegId::X16, RegId::X17, static_cast<uint8_t>(shift) & 0xF, v1);
  thread0->writeReg(RegId::X16, v2);
  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  //  read_reg() == reg + imm
  EXPECT_EQ(thread0->readReg(RegId::X17), (v2 << shift) - v1);
...
TEST_F(Slsubii, SignedLowerBoundShiftOne) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
TEST_F(Slsubii, SignedUpperBoundShiftOne) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
TEST_F(Slsubii, UnsignedLowerBoundShiftOne) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
TEST_F(Slsubii, UnsignedUpperBoundShiftOne) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
