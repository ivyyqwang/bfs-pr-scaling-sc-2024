...
class Subi : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Subi, Random) {
  ...
  int64_t v1 = (static_cast<int64_t>(rnd() << 48)) >> 48;
  int64_t v2 = rnd();
  int64_t v3 = rnd();

  inst = constrInstSubi(RegId::X16, RegId::X17, v1 & 0x000000000000FFFF);
  ...
  //  read_reg() == reg + imm
  EXPECT_EQ(static_cast<int64_t>(thread0->readReg(RegId::X17)), v2 - v1);
  EXPECT_EQ(static_cast<int64_t>(thread0->readReg(RegId::X16)), v2);
  EXPECT_EQ(cycle, basim::Cycles(1));
  // Expect to be empty.
}

/* Testing signed lower bound */
TEST_F(Subi, SignedLowerBound) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
  ...
  std::mt19937_64 rnd(std::random_device{}());
  int64_t v1 = static_cast<int64_t>((rnd() << 48) & 0x7FFFFFFFFFFFFFFF) >> 48;
...
TEST_F(Subi, SignedUpperBound) {
  ...
  std::mt19937_64 rnd(std::random_device{}());
  int64_t v1 = static_cast<int64_t>((rnd() << 48) | 0x8000000000000000) >> 48;
  int64_t v2 = 0x7FFFFFFFFFFFFFFF;
  int64_t v3 = rnd();
...
TEST_F(Subi, UnsignedLowerBound) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
...
TEST_F(Subi, UnsignedUpperBound) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
