...
class Vfill_32 : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Vfill_32, Random) {
  ...
  inst = constrInstVfill(2, v2 & 0x000000000000000F, RegId::X16, (v2 & 0x000000000000FFF0) >> 4);
  thread0->writeReg(RegId::X16, v1);

  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  EXPECT_EQ(thread0->readReg(RegId::X16), (v2 << 48) | (v2 << 16));
  // Expect to be empty.
}
...
TEST_F(Vfill_32, Max) {
  ...
  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  EXPECT_EQ(thread0->readReg(RegId::X16), 0xFFFF0000FFFF0000);
  // Expect to be empty.
}

/* Testing with empty value */
TEST_F(Vfill_32, Empty) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
