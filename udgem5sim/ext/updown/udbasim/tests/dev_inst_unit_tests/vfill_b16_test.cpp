...
class Vfill_b16 : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Vfill_b16, Random) {
  ...
  inst = constrInstVfill(3, v2 & 0x000000000000000F, RegId::X16, (v2 & 0x000000000000FFF0) >> 4);
  thread0->writeReg(RegId::X16, v1);

  Cycles cycle = decodeInst(inst).exe(*archstate, inst);
  // std::cout << "Cycles:" << cycle << std::endl;
  EXPECT_EQ(thread0->readReg(RegId::X16), (v2 << 48) | (v2 << 32) | (v2 << 16) | (v2));
  // Expect to be empty.
}

/* Testing with max value */
TEST_F(Vfill_b16, Max) {
  ThreadStatePtr thread0;
  uint8_t tid0 = tstable->getTID();
  thread0 = new ThreadState(tid0, 0, 0, 0);
  tstable->addtoTST(thread0);
