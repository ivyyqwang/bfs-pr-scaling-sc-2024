...
class Vgt_32 : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Vgt_32, Random) {
  ...
  inst = constrInstVgt(2, RegId::X16, RegId::X17, RegId::X18, mask);
  ...
  uint32_t v1_1 = static_cast<uint32_t>(v1 >> 32);
  uint32_t v2_1 = static_cast<uint32_t>(v2 >> 32);
  uint32_t v1_0 = static_cast<uint32_t>((v1 << 32) >> 32);
  uint32_t v2_0 = static_cast<uint32_t>((v2 << 32) >> 32);
