...
class Vgt_i32 : public ::testing::Test {
protected:
  // Setup arch state
  // push data
...
TEST_F(Vgt_i32, Random) {
  ...
  uint64_t v2 = rnd();
  uint64_t v3 = rnd();
  uint64_t mask = rnd() >> 62;

  inst = constrInstVgt(7, RegId::X16, RegId::X17, RegId::X18, mask);
  ...
  EXPECT_EQ(thread0->readReg(RegId::X18), (v3 & (~mask)) | (mask & (
    (static_cast<uint64_t>((static_cast<int32_t>(v1 >> 32)) > static_cast<int32_t>(v2 >> 32)) << 1) | 
    (static_cast<uint64_t>((static_cast<int32_t>((v1 << 32) >> 32)) > static_cast<int32_t>((v2 << 32) >> 32))))));
  // Expect to be empty.
}
