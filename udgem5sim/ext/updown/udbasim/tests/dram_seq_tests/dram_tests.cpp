...
int main(int argc, char* argv[], char* envp[]){
    if(argc != 4){
        cout << "Usage: ./dram_tests <numlanes> <numthreadsperlane> <nelemsperthread>" << endl;
        return 0;
    }
    // the args
    // 1. num lanes
...
for(int i = 1; i < 9; i++){
        std::string testname = "dram_tests_CHUNK" + std::to_string(i);
        cout << "Running test: " << testname << endl;
#ifdef BASIM
        cout << "BASIM" << endl;
        BASimRuntime_t *rt = new BASimRuntime_t(machine,
            "dram_tests.bin",
            0);
#else
#ifdef FASTSIM
...
void test_read_write(SimUDRuntime_t* rt, word_t* data, word_t* dram_dst, word_t* dram_src, int numlanes, int numthreadsperlane, int nelemsperthread){
    // ops/obs
