...
class flagTX : public ::testing::Test {
 protected:
...
TEST_F(flagTX, flagTX_0){
    acc0.initSetup(0, "testprogs/binaries/flag_tx_0.bin", 0); //(_pgbase,progfile,_lmbase)
    ...
    for(auto i = 0; i < numLanes; i++){
        ...
        EXPECT_TRUE(acc0.testReg(i, RegId::X17, 1));     //X17 = 1 in the last flag transition that should be executed
    }
}
