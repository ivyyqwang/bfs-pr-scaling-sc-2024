...
class SB_automatic_refill_TX : public ::testing::Test {
 protected:
...
TEST_F(SB_automatic_refill_TX, Basic_SB_automatic_refill_TX_0){
    acc0.initSetup(0, "testprogs/binaries/SB_automatic_refill_TX_0.bin", 0);
    int numop = 8;
    eventword_t ev0(0);
    ev0.setEventLabel(3156);
    ...
    for(auto i = 0; i < numLanes; i++){
        EXPECT_TRUE(acc0.testReg(i, RegId::X30, 64));
        EXPECT_TRUE(acc0.testReg(i, RegId::X31, 1));
        EXPECT_TRUE(acc0.testReg(i, RegId::X4, 1683627180608u));
        EXPECT_TRUE(acc0.testReg(i, RegId::X5,  ((i << 19) + 512)));
    }
}
