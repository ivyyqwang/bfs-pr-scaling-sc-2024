...
class UpDownObj(ClockedObject):
    type = 'UpDownObj'
    cxx_header = "updown_obj/updown_obj.hh"

    cpu_side = VectorResponsePort("CPU side port, receives requests")
    mem_side = VectorRequestPort("Memory side port, sends requests")
    #udin_side = VectorResponsePort("UD In side port, receives UD requests")
    ...
    dblksize = Param.Int(64, "Number of Lanes in updown")
