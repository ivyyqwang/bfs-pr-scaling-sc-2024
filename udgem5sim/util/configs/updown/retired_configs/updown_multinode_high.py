...
for i in range(nnodes):
    ...
    systems[i].membus = [StackXBar(intlv_bits=intlv_bits, intlv_low_bit=7) for x in range(nstacks)]
    systems[i].clxbar = [CLXBar(ctrl_base_addr = node_ud_ctrl_addr[i].start + ud_ctrl_size*x*nuds,
                                  ...
                                  cluster_id = x,
                                  num_channels = 1,
                                  frontend_latency = int(cluster_latency/2),
                                  forward_latency = int(cluster_latency/2),
    ...
    systems[i].cpuxbar = [CPUXBar(ctrl_base_addr = node_ud_ctrl_addr[i].start + ud_ctrl_size*x*nuds,
                                  ...
                                  cluster_id = x,
                                  num_channels = 1,
                                  frontend_latency = int(node_latency/2),
                                  forward_latency = int(node_latency/2),
                                  clk_domain=switch_clk_domain) 
                          for x in range(nstacks)]
    ...
    #Trying a bridge between Crossbars
    cpubridges =[]
    for j in range(nstacks):
        for k in range(np):
            lranges=[]
            lranges.append(AddrRange(ud_sp_base_addr + j* ud_sp_size*nuds, ud_sp_base_addr + j*ud_sp_size*nuds + ud_sp_size*nuds))
            lranges.append(AddrRange(ud_ctrl_base_addr + j* ud_ctrl_size*nuds, ud_ctrl_base_addr + j * ud_ctrl_size*nuds + ud_ctrl_size*nuds))
    ...
    for j in range(nstacks):
        ...
        for k in range(np):
            ...
            systems[i].cpuxbar[j].cpu_in_ports = systems[i].cpubridge[j*np+k].mem_side_port
        systems[i].clxbar[j].cpu_in_ports = systems[i].cpuxbar[j].mem_side_ports
        systems[i].clxbar[j].mem_side_ports = systems[i].cpuxbar[j].clx_in_ports
        for k in range(nstacks):
            if j != k:
                systems[i].cpuxbar[j].mem_side_ports = systems[i].cpuxbar[k].cpu_side_ports
