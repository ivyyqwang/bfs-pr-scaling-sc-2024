...
for i in range(nnodes):
    ...
    if nstacks > 1:
        ...
        for j in range(nstacks):
            ...
            # cluster out bridge 
                ...
                clxbridges.append(Bridge(req_size=16384, resp_size=16384, ranges=lranges, clk_domain=switch_clk_domain,delay='10ns'))
        systems[i].clxbridge=clxbridges
    ...
    for j in range(nstacks):
        ...
        systems[i].clxbar[j].cpu_in_ports = systems[i].interxbar[j].mem_side_ports
        systems[i].clxbar[j].mem_side_ports = systems[i].interxbar[j].clx_in_ports
        for k in range(nstacks):
            if j != k:
                systems[i].interxbar[j].mem_side_ports = systems[i].clxbridge[k*(nstacks-1) + l[k]].cpu_side_port
                systems[i].clxbridge[k*(nstacks-1) + l[k]].mem_side_port = systems[i].interxbar[k].cpu_side_ports
                l[k] += 1
